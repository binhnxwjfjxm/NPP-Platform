-- Issue #1099 Lô 2: persist per-line stocktake reason/note and lock counted round history.

ALTER TABLE inventory.stocktake_lines
  ADD COLUMN IF NOT EXISTS count_reason text NULL,
  ADD COLUMN IF NOT EXISTS count_note text NULL;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conname = 'stocktake_lines_count_reason_length'
       AND conrelid = 'inventory.stocktake_lines'::regclass
  ) THEN
    ALTER TABLE inventory.stocktake_lines
      ADD CONSTRAINT stocktake_lines_count_reason_length
      CHECK (count_reason IS NULL OR char_length(btrim(count_reason)) BETWEEN 1 AND 500);
  END IF;

  IF NOT EXISTS (
    SELECT 1
      FROM pg_constraint
     WHERE conname = 'stocktake_lines_count_note_length'
       AND conrelid = 'inventory.stocktake_lines'::regclass
  ) THEN
    ALTER TABLE inventory.stocktake_lines
      ADD CONSTRAINT stocktake_lines_count_note_length
      CHECK (count_note IS NULL OR char_length(btrim(count_note)) BETWEEN 1 AND 2000);
  END IF;
END;
$$;

COMMENT ON COLUMN inventory.stocktake_lines.count_reason IS
  'Reason captured for this exact stocktake line and counting round.';
COMMENT ON COLUMN inventory.stocktake_lines.count_note IS
  'Operator note captured for this exact stocktake line and counting round.';
