...
-- fulfillment projection, and the database backstop used by canonical movement posting.
...
INSERT INTO shared.permission_catalog (
  ...
  'Cho phép người dùng thực hiện xuất kho làm tồn xuống âm tại kho đã bật chính sách tương ứng. Hệ thống vẫn kiểm tra phạm vi kho và các điều kiện nghiệp vụ.',
  true,
  now()
)
...
-- Preserve the latest fulfillment guard contract from migration 094 and extend it narrowly
-- for the explicit negative-issued projection. Hold and release contexts may not touch it.
...
BEGIN
  ...
  IF write_context = 'delivery_issue_service' THEN
    IF NEW.allocated_base_quantity IS DISTINCT FROM OLD.allocated_base_quantity
       ...
       OR NEW.negative_issued_base_quantity > NEW.backordered_base_quantity THEN
...
-- for a canonical SALES_DELIVERY_ISSUE and only when the current transaction carries a
-- server-owned context that matches the movement, installation and warehouse exactly.
...
BEGIN
  ...
  IF trusted_context IS NULL
     ...
     OR trusted_context->>'source' IS DISTINCT FROM 'SERVER_POLICY'
     OR trusted_context->>'decision' IS DISTINCT FROM 'ALLOW'
     OR trusted_context->>'permissionKey' IS DISTINCT FROM 'core.inventory.negative-stock.issue'
