...
export function PrinterSettingsPanel({ initialSettings, onSaved, onClose, onNotice, onError }: Props) {
  ...
  const [directHelpOpen, setDirectHelpOpen] = useState(false);
  ...
  async function testPrinter() {
    ...
    if (draft.method === 'SYSTEM') {
      ...
      onNotice('Đã mở giao diện in của thiết bị. Phiếu được định dạng theo khổ đã chọn; máy in/AirPrint quyết định khổ giấy thực tế có hỗ trợ hay không.');
      return;
    }
    if (!directReady) {
  ...
  return <div className="printer-settings">
    <section className="printer-setting-section printer-method-step">
      <header><strong>Chọn cách in</strong><small>Chọn phương thức in phù hợp với thiết bị.</small></header>
      <div className="printer-methods" role="radiogroup" aria-label="Phương thức in mặc định">
        <button type="button" role="radio" aria-checked={draft.method === 'DIRECT_WIFI'} className={`${draft.method === 'DIRECT_WIFI' ? 'active ' : ''}${!directReady ? 'unavailable' : ''}`.trim()} onClick={chooseDirectMethod}><span aria-hidden="true">Wi‑Fi</span><strong>In Wi‑Fi trực tiếp</strong><small>{checkingBridge ? 'Đang kiểm tra thiết bị…' : windowsRelay ? 'Qua Retail Print trên Windows' : directReady ? 'In thẳng tới máy đã lưu' : 'Chạm để xem cách sử dụng'}</small></button>
        <button type="button" role="radio" aria-checked={draft.method === 'SYSTEM'} className={draft.method === 'SYSTEM' ? 'active' : ''} onClick={() => { setDirectHelpOpen(false); setDraft((current) => ({ ...current, method: 'SYSTEM' })); }}><span aria-hidden="true">↗</span><strong>In bằng hệ thống</strong><small>AirPrint hoặc máy in đã cài trên điện thoại</small></button>
      </div>
    ...
    {draft.method === 'DIRECT_WIFI' ? <section className="printer-setting-section printer-device-step">
      <header><strong>Máy in Wi‑Fi/LAN</strong><small>{windowsRelay ? 'Chọn máy Windows và máy in cần sử dụng.' : 'Điện thoại và máy in cần ở cùng mạng.'}</small></header>
      ...
      <button className="secondary-action printer-discover" type="button" disabled={!capabilities.discovery || discovering} onClick={() => void findPrinters()}>{discovering ? 'Đang tải máy Windows…' : windowsRelay ? 'Tải máy Windows' : 'Tìm máy in'}</button>
      {found.length ? <div className="printer-results" aria-label="Máy in tìm thấy">{found.map((profile) => <button type="button" className={draft.profile?.id === profile.id ? 'selected' : ''} key={profile.id} onClick={() => choosePrinter(profile)}><span aria-hidden="true">▣</span><span><strong>{profile.name}</strong><small>{profile.lastVerifiedStatus === 'READY' ? 'Đã sẵn sàng' : 'Chạm để chọn'}</small></span><b aria-hidden="true">›</b></button>)}</div> : null}
    ...
    <section className="printer-setting-section printer-compact-options printer-options-step">
      <header><strong>Cài đặt in</strong><small>Chọn khổ giấy và cách in cho thiết bị này.</small></header>
    ...
    <div className="printer-test-row printer-status-step"><span><strong>Trạng thái</strong><small>{status}</small></span><button className="secondary-action" type="button" disabled={testing} onClick={() => void testPrinter()}>{testing ? 'Đang kiểm tra…' : 'In thử'}</button></div>

    <div className="settings-sheet-actions printer-settings-actions"><button className="secondary-action" type="button" onClick={onClose}>Hủy</button><button className="primary-action" type="button" onClick={save}>Lưu thiết lập</button></div>
  </div>;
}
