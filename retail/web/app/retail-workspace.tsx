...
function normalizePrintFontSizePercent(value: unknown) { const percent = Number(value); return Number.isInteger(percent) ? Math.max(80, Math.min(140, percent)) : 100; }
...
export default function RetailWorkspace({ initialTab = 'home', inventoryAvailable = false, onOpenInventory, onTabChange }: RetailWorkspaceProps) {
    ...
    const promotionCount = cart.reduce((count, line) => {
        const preview = prices[line.id];
        if (!preview || preview.inputKey !== priceInputKey(line.id, line.quantity)) return count;
        return count + (preview.appliedRules ?? []).filter((rule) => rule.priceListType === 'PROMOTION').length;
    }, 0);
    ...
    function applyTemplate(template: PrintTemplate) {
        ...
        setTemplateFontSizePercent(normalizePrintFontSizePercent(template.fontSizePercent));
    }
    ...
    function buildOrderPrintPayload(template: PrintTemplate | null, settings: PrinterSettings) {
        ...
        return buildSalesOrderPrintPayload({
            ...
            fontSizePercent: normalizePrintFontSizePercent(template?.fontSizePercent),
    ...
    async function savePrintTemplate() {
        ...
        try {
            const next = await api<PrintTemplate>(`/api/retail/print-templates/${printTemplate.documentType}/${printTemplate.templateCode}`, {
                ...
                body: JSON.stringify({ pageSize: printTemplate.pageSize, fontSizePercent: templateFontSizePercent, visibleFieldKeys: printTemplate.visibleFieldKeys, heading: templateHeading.trim() || null, title: templateTitle.trim() || null, subtitle: templateSubtitle.trim() || null, expectedUpdatedAt: printTemplate.updatedAt ?? null }),
            });
