...
export default function RetailWorkspace() {
    ...
    {activeTab === 'settings' ? <section className="settings-workspace retail-page">
      ...
      <div className="settings-list">
        ...
        <button className="settings-row" type="button" onClick={() => openSettings('printer')}><span className="settings-icon" aria-hidden="true">▣</span><span><strong>Máy in</strong><small>Khổ mặc định: {printPaper}</small></span><b aria-hidden="true">›</b></button>
    ...
    {settingsPanel ? <section className="dialog-backdrop settings-sheet-backdrop" role="dialog" aria-modal="true" aria-label={settingsPanel === 'account' ? 'Tài khoản' : settingsPanel === 'printer' ? 'Máy in' : settingsPanel === 'template' ? 'Mẫu phiếu' : 'Đăng xuất'}><div className="settings-sheet sheet-enter">
      <header><div><p className="section-kicker">CÀI ĐẶT</p><h2>{settingsPanel === 'account' ? 'Tài khoản' : settingsPanel === 'printer' ? 'Máy in' : settingsPanel === 'template' ? 'Mẫu phiếu' : 'Đăng xuất'}</h2></div><button className="text-action" type="button" onClick={() => setSettingsPanel(null)}>Đóng</button></header>
      ...
      {settingsPanel === 'printer' ? <><label className="settings-control">Khổ in mặc định<select value={draftPrintPaper} onChange={(event) => setDraftPrintPaper(event.target.value as PrintPaper)}><option value="A4">A4</option><option value="A5">A5</option><option value="80mm">80 mm</option><option value="58mm">58 mm</option></select></label><p className="settings-help">Máy in Wi-Fi được chọn trong hộp thoại in của thiết bị.</p><div className="settings-sheet-actions"><button className="secondary-action" type="button" onClick={() => setSettingsPanel(null)}>Hủy</button><button className="primary-action" type="button" onClick={() => { changePaper(draftPrintPaper); setSettingsPanel(null); setNotice('Đã thiết lập khổ in mặc định trên thiết bị này.'); }}>Thiết lập</button></div></> : null}
