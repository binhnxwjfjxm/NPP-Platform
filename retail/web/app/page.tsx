...
export default function RetailHomePage() {
  ...
  function priceInputKey(variantId: string, quantity: string) {
    return [boot?.settings.defaultSalesChannelId ?? '', customerMode, customerMode === 'EXISTING' ? customerId : '', variantId, quantity].join(':');
  }
  ...
  function orderPayload() {
    ...
    return { sourceType: 'MANUAL', customerMode, ...(customerMode === 'EXISTING' ? { customerId } : {}), warehouseId, salesChannelId: boot.settings.defaultSalesChannelId, deliveryMode: 'PICKUP', collectionPolicy: policy, currency: 'VND', ...(order ? { expectedRevision: order.revision } : {}), lines: cart.map((line) => {
  ...
  async function save() {
    ...
    try {
      ...
      setOrder(next); setCart([]); setPrices({}); setEditPickup(false); setNotice(order ? 'Đã lưu thay đổi đơn và giữ nguyên trạng thái Đã chốt.' : 'Đã lập đơn tại quầy.');
  ...
  function resetEntry() {
    setOrder(null); setCart([]); setPrices({}); setEditPickup(false); setAvailable([]); setNotice(null); setError(null); setActiveTab('entry');
  }
