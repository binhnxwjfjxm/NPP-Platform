...
test('Khả dụng khi sửa đơn dùng preview theo kho và loại chính đơn hiện tại', async () => {
  ...
  assert.match(service, /String\(sourceLines\[0\]\.warehouse_id\) !== warehouseId/);
  assert.match(service, /RETAIL_AVAILABILITY_WAREHOUSE_MISMATCH/);
  assert.match(service, /excludingSalesOrderId = salesOrderId/);
  assert.match(service, /getWarehouseAvailableQuantity/);
});

test('điều hướng Retail có bốn mục và đăng xuất không quay lại lịch sử trình duyệt', async () => {
  const page = await read('app/page.tsx');
  ...
  assert.match(page, />Tài khoản<\/button>/);
  assert.match(page, /action="\/api\/auth\/logout"/);
  assert.doesNotMatch(page, /window\.history\.back/);
});

test('in phiếu đọc mẫu Công Ty và hỗ trợ A4 A5 80mm 58mm không đưa ảnh vào chứng từ', async () => {
  const [page, gateway] = await Promise.all([read('app/page.tsx'), read('app/api/retail/[...segments]/route.ts')]);
  assert.match(page, /\/api\/retail\/print-templates/);
  assert.match(gateway, /\/api\/document-print-templates/);
...
test('ba thao tác đơn đúng thứ tự, giữ một hàng trên mobile và không che tổng tiền', async () => {
  const [page, css] = await Promise.all([read('app/page.tsx'), read('app/retail-lot7.css')]);
  const actions = page.slice(page.indexOf('className="order-action-bar"'));
  assert.ok(actions.indexOf('✎ Sửa đơn') < actions.indexOf('▣ In phiếu'));
  assert.ok(actions.indexOf('▣ In phiếu') < actions.indexOf('⌑ Xuất kho'));
  assert.match(css, /\.retail-lot7 \.order-action-bar \{[^}]*grid-template-columns: repeat\(3,minmax\(0,1fr\)\)/);
  assert.match(css, /@media \(max-width: 640px\)[\s\S]*?\.retail-lot7 \.order-action-bar \{[^}]*grid-template-columns: repeat\(3,minmax\(0,1fr\)\)/);
  assert.match(css, /padding-bottom: calc\(190px \+ env\(safe-area-inset-bottom\)\)/);
});
