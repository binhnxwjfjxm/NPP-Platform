...
test('Lô 6 có điều hướng Retail, thanh thao tác đáy và mở lại đơn Giao tại quầy', async () => {
  ...
  assert.match(page, /type RetailTab = 'home' \| 'entry' \| 'orders' \| 'settings'/); assert.match(page, /Trang chủ/); assert.match(page, /Lên đơn/); assert.match(page, /Đơn hàng/); assert.match(page, /Cài đặt/); assert.match(page, /placeholder="Nhập mã đơn, khách hàng, số điện thoại"/); assert.match(page, /const ORDER_BATCH_SIZE = 100/); assert.match(page, /Xem thêm đơn hàng/); assert.match(page, /item\.deliveryMode === 'PICKUP'/); assert.match(page, /order-action-bar/); assert.match(page, /bottom-nav/); assert.match(styles, /\.order-action-bar \{ position: fixed/); assert.match(styles, /\.bottom-nav \{ position: fixed/);
});
...
test('Lô 6 chỉ dùng giá preview đúng đầu vào và không báo lỗi sau khi thao tác đã thành công', async () => { const page = await readWorkspace(); assert.match(page, /type CachedPricePreview = PricePreview & \{\s*inputKey: string;?\s*\}/); assert.match(page, /preview\?\.inputKey === priceInputKey\(line\.id, line\.quantity\)/); assert.match(page, /void refreshOrders\(\)\.catch\(\(\) => undefined\)/); assert.match(page, /setPrices\(\{\}\)/); });
