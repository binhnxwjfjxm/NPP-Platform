...
test('Lô 7 có ảnh R2, giá picker, trạng thái Đã xuất kho và phương thức thu tiền', async () => { const [page, css] = await Promise.all([readWorkspace(), read('app/retail-lot7.css')]); assert.match(page, /app-customer\/products/); assert.match(page, /Đang tính giá/); assert.match(page, /return 'Đã xuất kho'/); assert.match(page, /PaymentMethod = 'CASH' \| 'BANK_TRANSFER'/); assert.match(page, /paymentMethod/); assert.match(css, /compact-product-card/); assert.match(css, /lot7-payment/); });
