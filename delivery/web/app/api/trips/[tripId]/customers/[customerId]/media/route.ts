...
import { getMyCustomerMedia } from '../../../../../../../lib/core-api';
...
export async function GET(
  request: NextRequest,
  context: { params: { tripId: string; customerId: string } },
) {
  ...
  try {
    const data = await getMyCustomerMedia(user, context.params.tripId, context.params.customerId);
    return NextResponse.json(
      { data },
    ...
    return errorResponse(
      details.code || 'DELIVERY_CUSTOMER_MEDIA_REQUEST_FAILED',
      details.message || 'Không tải được ảnh khách hàng',
