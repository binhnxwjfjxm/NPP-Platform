...
import { DELIVERY_SESSION_COOKIE } from './delivery-session';

export const DELIVERY_INTERNAL_SOURCE_APP = 'delivery-web';
...
export type DeliveryInternalAuthResult<T> = Readonly<{
  ok: boolean;
  status: number;
  data?: T;
  code?: string;
...
export function deliveryCoreBaseUrl(): string {
  const raw = process.env.CORE_API_INTERNAL_URL?.trim();
  if (!raw) throw new Error('DELIVERY_CORE_NOT_CONFIGURED');
  let url: URL;
  try {
    url = new URL(raw);
  } catch {
...
export function readDeliverySessionToken(): string | null {
  return cookies().get(DELIVERY_SESSION_COOKIE)?.value?.trim() || null;
}

export function requireDeliverySessionToken(): string {
  const token = readDeliverySessionToken();
  if (!token?.startsWith('nppusr.')) throw new Error('DELIVERY_AUTH_REQUIRED');
  return token;
}

export async function requestDeliveryInternalAuth<T>(
...
): Promise<DeliveryInternalAuthResult<T>> {
  let baseUrl: string;
  try {
    baseUrl = deliveryCoreBaseUrl();
  } catch (error) {
  ...
  try {
    const response = await fetch(`${baseUrl}${path}`, {
      ...
      headers: {
        ...
        'x-request-id': `delivery_auth_${randomUUID()}`,
