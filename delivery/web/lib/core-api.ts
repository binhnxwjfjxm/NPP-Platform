...
export type DeliveryCustomerMediaItem = Readonly<{
  id: string;
  customerId: string;
  sourceApp: string;
  mimeType: string;
...
export type DeliveryCustomerMediaResponse = Readonly<{
  media: readonly DeliveryCustomerMediaItem[];
  maxPhotos: number;
}>;
...
async function requestCore<T>(path: string, user: DeliveryUser): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const error = new Error(errorPayload.error?.message || errorPayload.error?.code || 'DELIVERY_CORE_REQUEST_FAILED');
    (error as Error & { status?: number; code?: string }).status = response.status;
    (error as Error & { status?: number; code?: string }).code = errorPayload.error?.code;
    throw error;
  }
...
export function getMyCustomerMedia(
  user: DeliveryUser,
  tripId: string,
  customerId: string,
): Promise<DeliveryCustomerMediaResponse> {
  if (!UUID_PATTERN.test(tripId)) throw new Error('INVALID_TRIP_ID');
  if (!UUID_PATTERN.test(customerId)) throw new Error('INVALID_CUSTOMER_ID');
  return requestCore<DeliveryCustomerMediaResponse>(
    `/api/logistics/driver/trips/${encodeURIComponent(tripId)}/customers/${encodeURIComponent(customerId)}/media`,
    user,
  );
}
