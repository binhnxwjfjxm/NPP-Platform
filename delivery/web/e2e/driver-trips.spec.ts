...
test('tài xế xem danh sách và chi tiết chuyến trên mobile', async ({ page }) => {
  await page.goto('/');
  ...
  await expect(page.getByText('Ghi kết quả giao và bằng chứng giao hàng thuộc phần tiếp theo.')).toBeVisible();
  ...
  await expect(page.getByText(/POD\/GPS và thu tiền chưa thuộc slice hiện tại/)).toBeVisible();
});
