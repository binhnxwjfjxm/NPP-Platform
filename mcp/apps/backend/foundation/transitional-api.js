...
export async function handleTransitionalApi(req, url, context, config, { fetchImpl = fetch } = {}) {
  ...
  if (sessionLifecycle && method === "PATCH") return saveRouteSessionUpdate(req, decodePathId(sessionLifecycle[1], "invalid_session_id"), context, config, fetchImpl);
  if (sessionLifecycle && method === "DELETE") return removeEmptyRouteSession(decodePathId(sessionLifecycle[1], "invalid_session_id"), context, config, fetchImpl);
