...
export async function legacySupabaseRequest(
  config,
  path,
...
export function legacySupabaseRest(config, resource, options = {}) {
  return legacySupabaseRequest(config, `/rest/v1/${resource}`, options);
}

export function legacySupabaseRpc(config, name, args, options = {}) {
  return legacySupabaseRequest(config, `/rest/v1/rpc/${name}`, {
    method: "POST",
    body: args,
    ...options
  });
