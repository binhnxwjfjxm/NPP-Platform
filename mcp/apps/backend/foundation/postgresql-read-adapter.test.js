...
test("PostgreSQL reads use an explicit schema and force configured installation", async () => {
  const calls = bindCapture([{ id: "row-1", __canonical_google_maps_url: null }]);
  ...
  assert.deepEqual(rows, [{ id: "row-1", google_maps_url: null }]);
...
test("linked route-customer read prefers canonical shared.customer_addresses location_url", async () => {
  ...
  assert.match(calls[0].sql, /THEN customer_address\.location_url/);
  assert.match(calls[0].sql, /ELSE route_customer\.google_maps_url/);
});
