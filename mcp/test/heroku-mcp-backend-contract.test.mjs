...
const requestContext = await read("mcp/apps/backend/foundation/request-context.js");
...
test("MCP backend runtime stays on bootstrap.js with locked PostgreSQL dependencies", () => {
  ...
  assert.match(requestContext, /\.\.\/\.\.\/\.\.\/\.\.\/packages\/contracts\/index\.js/);
  ...
  assert.ok(dockerfile.includes("COPY mcp/apps/backend/package.json mcp/apps/backend/package-lock.json ./mcp/apps/backend/"));
  assert.ok(dockerfile.includes("COPY packages/contracts ./packages/contracts"));
  assert.ok(dockerfile.includes("RUN npm ci --omit=dev --ignore-scripts"));
  assert.ok(dockerfile.includes("COPY mcp/apps/backend/bootstrap.js mcp/apps/backend/server.js ./"));
  assert.match(dockerfile, /COPY mcp\/apps\/backend\/foundation \.\/foundation/);
...
test("manual Heroku MCP deployment performs PostgreSQL preflight, backup, migration and isolated rollback", () => {
  ...
  assert.match(deployScript, /heroku container:push web -a "\$HEROKU_APP_NAME" --context-path \.\.\/\.\.\/\.\./);
...
test("Heroku MCP CI builds, verifies and smokes backend with frontend build fixtures", () => {
  ...
  assert.match(ciWorkflow, /packages\/contracts\/\*\*/);
  ...
  assert.match(ciWorkflow, /docker build -f mcp\/apps\/backend\/Dockerfile \. -t heroku-mcp-backend:/);
