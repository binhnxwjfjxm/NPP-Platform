...
import { currentMcpLocalUserId, mcpLocalIdentity, useMcpLocalUserId } from "./mcp-local-identity";
...
let memorySnapshot: { userId: string; value: McpShellSnapshot } | null = null;

function scope(userId: string): LocalReadScope {
  return { ...mcpLocalIdentity(userId), resource: RESOURCE, schemaVersion: SCHEMA_VERSION };
}
...
export async function clearMcpLocalReadForCurrentUser() {
  ...
  try {
    if (userId) await cache.clearIdentity(mcpLocalIdentity(userId));
  } catch {
    // Logout must continue even when IndexedDB is unavailable.
  } finally {
    memorySnapshot = null;
  }
}
...
export function useMcpShellSnapshot() {
  ...
  const remembered = userId && memorySnapshot?.userId === userId ? memorySnapshot.value : null;
  const [snapshot, setSnapshot] = useState<McpShellSnapshot | null>(() => remembered);
  ...
  const applySnapshot = useCallback((next: McpShellSnapshot, nextSource: "local" | "live") => {
    if (!userId) return;
    memorySnapshot = { userId, value: next };
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!aliveRef.current) return;
      const message = cause instanceof Error ? cause.message : "MCP_LOCAL_READ_UNAVAILABLE";
      setError(message);
  ...
  useEffect(() => {
    ...
    const existing = memorySnapshot?.userId === userId ? memorySnapshot.value : null;
    ...
    const start = async () => {
      const activeScope = scope(userId);
      ...
      if (cached) applySnapshot(cached, "local");
      try {
