...
export async function loadOrdersLocalData(): Promise<ApiResult<OrderDto[]>> {
  const [legacyOrdersResult, coreOrders, customerQueue] = await Promise.all([
    loadOrdersResult(),
    loadOwnedCoreSalesOrders(),
    loadCustomerOnboardingQueue()
  ]);
  const routeNames = new Map(customerQueue.map((item) => [item.routeCustomerId, item.routeName || "MCP"]));
  ...
  return { ...legacyOrdersResult, data: mergedOrders, receivedAt: new Date().toISOString() };
}
