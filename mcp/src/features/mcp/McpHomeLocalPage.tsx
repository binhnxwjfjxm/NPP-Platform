...
function LoadingState() {
  return <AppShell activeHref="/mcp">
    <PageHeader eyebrow="MCP" title="Quản lý đi thị trường" subtitle="Đang mở dữ liệu đã lưu và cập nhật số liệu mới." />
    <section className="dashboard-section" aria-busy="true"><div className="empty-inline">Đang tải dữ liệu MCP...</div></section>
  </AppShell>;
}

function renderDesktopModule(item: (typeof DESKTOP_MODULES)[number]) {
  return <Link className={[styles.card, styles[item.tone]].join(" ")} href={item.href} key={item.href + item.title} prefetch>
    <span className={styles.icon} aria-hidden="true">{item.icon}</span>
    <span className={styles.content}><strong>{item.title}</strong><small>{item.description}</small></span>
    <span className={styles.cta}>{item.cta}</span>
  </Link>;
}

export function McpHomeLocalPage() {
  const { snapshot, source, loading, refreshing, error, refresh } = useMcpShellSnapshot();
  if (!snapshot && loading) return <LoadingState />;
  if (!snapshot) {
    return <AppShell activeHref="/mcp">
      <PageHeader eyebrow="MCP" title="Quản lý đi thị trường" subtitle="Chưa tải được dữ liệu tuyến." />
      <section className="dashboard-section" role="alert"><div className="empty-inline"><strong>Chưa tải được dữ liệu</strong><br />Vui lòng thử lại.<div><button className="button" type="button" onClick={() => void refresh()}>Tải lại</button></div></div></section>
    </AppShell>;
  }

  const routes = snapshot.routesData.routes;
  ...
  return <AppShell activeHref="/mcp">
    <section className={styles.mobileFlow} data-mcp-mobile-flow="true" aria-label="Tác nghiệp MCP">
      <Link className={styles.primaryAction} data-mcp-primary-action="true" href="/visits" prefetch>
        ...
        <span className={styles.primaryCopy}><strong>Đi tuyến hôm nay</strong><small>{activeRoutes} tuyến có thể đi · {plannedCustomers} điểm bán</small></span>
      ...
      <nav className={styles.mobileActionList} aria-label="Quản lý MCP">
        {mobileActions.map((item) => <Link className={styles.mobileAction} data-mcp-mobile-action="true" href={item.href} key={item.href} prefetch>
          <span className={styles.mobileActionIcon} aria-hidden="true">{item.icon}</span>
          <span className={styles.mobileActionCopy}><strong>{item.title}</strong><small>{item.meta}</small></span>
          <span className={styles.chevron} aria-hidden="true">›</span>
        </Link>)}
      </nav>
      <div className={styles.mobileUtility} aria-label="Xuất dữ liệu MCP"><span className={styles.mobileUtilityLabel}><span aria-hidden="true">⇩</span><strong>Xuất dữ liệu</strong></span><ExportMenu label="Chọn" /></div>
    </section>
