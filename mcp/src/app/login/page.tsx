...
type Props = Readonly<{
  searchParams?: Readonly<{ returnTo?: string; error?: string; state?: string }>;
}>;
...
export default function LoginPage({ searchParams }: Props) {
  ...
  return (
    <main className={styles.page}>
      <section className={styles.card} aria-labelledby="mcp-login-title">
        <header className={styles.brand}>
          <strong>NPP MCP FIELD</strong>
          <span>Ứng dụng bán hàng thị trường</span>
        </header>
        ...
        <div className={styles.content}>
          {mode === "credentials" ? (
            ...
            <div className={styles.verificationStage}>
              ...
              <h1 id="mcp-login-title">Xác minh thiết bị</h1>
              ...
              <form className={styles.form} action="/api/auth/login" method="post" onSubmit={handleVerificationSubmit}>
                <input type="hidden" name="returnTo" value={returnTo} />
                <label key={codeAttempt} className={`${styles.field} ${codeAttempt > 0 ? styles.codeError : ""}`}>
