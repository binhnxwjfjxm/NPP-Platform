...
export async function insertCustomer(client, { installationId, code, name, address, phone, email, createdBy }) {
  const id = randomUUID();
...
export async function getCustomerById(client, { id }) {
  const result = await client.query(
...
export async function getCustomerByIdForInstallationForShare(client, { id, installationId }) {
  const result = await client.query(
    `SELECT id, installation_id, code, name, address, phone, email, is_active, created_at, updated_at, created_by, updated_by
     FROM shared.customers
     WHERE id = $1 AND installation_id = $2
...
export async function listCustomersForInstallation(client, { installationId, active, limit = 100, offset = 0 }) {
...
export async function updateCustomer(client, { id, installationId, name, address, phone, email, updatedBy, expectedUpdatedAt = null }) {
  const params = [name, address || null, phone || null, email || null, updatedBy, id, installationId];
  let query = `UPDATE shared.customers
     SET name = $1,
         address = $2,
         phone = $3,
         email = $4,
...
export async function updateCustomerActiveStatus(client, { id, installationId, isActive, updatedBy, expectedUpdatedAt = null }) {
