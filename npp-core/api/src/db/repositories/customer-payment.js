...
export async function getCustomerAndWarehouse(client, {
  installationId,
  customerId,
  warehouseId,
}) {
  const result = await client.query(
    `SELECT customer.id AS customer_id,
       ...
       JOIN shared.warehouses warehouse
         ON warehouse.installation_id = customer.installation_id
...
export async function insertCustomerPayment(client, input) {
  const result = await client.query(
    ...
    [
      ...
      input.paymentDate,
      input.customerCodeSnapshot,
      input.customerNameSnapshot,
      input.warehouseCodeSnapshot,
      input.warehouseNameSnapshot,
      input.currencyCode,
      input.amount,
      input.postedAt,
      input.actorId,
      input.documentNumberAllocationId,
      input.paymentMethod,
...
export async function listCustomerPayments(client, {
  installationId,
  warehouseIds,
  customerId = null,
  warehouseId = null,
...
export async function getCustomerPaymentById(client, {
  installationId,
  id,
  warehouseIds,
  forUpdate = false,
    ...
    client.query(
      `SELECT *
        ...
        WHERE installation_id = $1
          AND receivable_document_id = $2::uuid
...
export async function listOpenAllocationTargets(client, {
  ...
  const result = await client.query(
    `SELECT document.*,
      ...
      ORDER BY document.source_document_date, document.posted_at, document.id
      LIMIT $6`,
...
export async function createAllocation(client, input) {
  const result = await client.query(
    `SELECT * FROM accounting.create_receivable_allocation(
...
export async function reverseAllocation(client, input) {
  const result = await client.query(
    `SELECT * FROM accounting.reverse_receivable_allocation(
...
export async function reverseCustomerPayment(client, input) {
  const result = await client.query(
    `SELECT * FROM accounting.reverse_customer_payment(
       $1::text,$2::uuid,$3::text,$4::timestamptz,$5::text
     )`,
    [
      input.installationId,
      input.paymentId,
...
export async function getAllocationById(client, {
  installationId,
  id,
  warehouseIds,
  forUpdate = false,
