      ...
      LEFT JOIN LATERAL (
        SELECT sum(delivery_line.delivery_base_quantity) AS claimed_base_quantity
         ...
         WHERE delivery_line.installation_id = allocation.installation_id
           ...
           AND delivery_order.status IN ('draft', 'ready_to_dispatch')
      ) claimed ON true
...
export async function getEligibleAllocationForUpdate(client, {
  installationId,
  allocationId,
}) {
  const result = await client.query(
...
export async function getDeliveryOrderByCreateKey(client, {
  installationId,
  idempotencyKey,
}) {
  const result = await client.query(
  ...
  const result = await client.query(
    `SELECT
      ...
      LEFT JOIN LATERAL (
        SELECT
          count(*) AS line_count,
          sum(line.delivery_base_quantity) AS total_base_quantity
...
export async function listDeliveryOrderLines(client, {
  installationId,
  deliveryOrderId,
}) {
  const result = await client.query(
...
export async function listDeliveryOrderEvents(client, {
  installationId,
  deliveryOrderId,
}) {
  const result = await client.query(
    `SELECT *
       FROM sales.delivery_order_events
      WHERE installation_id = $1
...
export async function getDeliveryOrderEventByKey(client, {
  installationId,
  idempotencyKey,
}) {
  const result = await client.query(
...
export async function insertDeliveryOrder(client, data) {
  const result = await client.query(
    ...
    [
      ...
      data.customerCode,
      data.customerName,
      JSON.stringify(data.destinationSnapshot ?? {}),
      data.warehouseCode,
      data.warehouseName,
...
export async function insertDeliveryOrderLine(client, data) {
  const result = await client.query(
    ...
    [
      ...
      data.itemName,
      data.unitCode,
      data.packedBaseQuantity,
      data.deliveryBaseQuantity,
      data.actorId,
    ],
  );
  return result.rows[0] ?? null;
