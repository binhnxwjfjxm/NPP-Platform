...
export async function listWarehousesForBranch(client, { branchId, active, limit = 100, offset = 0 }) {
  let query = `SELECT id, installation_id, branch_id, code, name, warehouse_type, is_active, created_at, updated_at, created_by, updated_by
               FROM shared.warehouses
               WHERE branch_id = $1`;
...
export async function updateWarehouse(client, { id, installationId, name, warehouseType, updatedBy }) {
  const now = new Date().toISOString();
  ...
  const result = await client.query(
    `UPDATE shared.warehouses
     ...
     RETURNING id, installation_id, branch_id, code, name, warehouse_type, is_active, created_at, updated_at, created_by, updated_by`,
    [name, warehouseType, now, updatedBy, id, installationId],
  );

  return result.rows[0] || null;
}

export async function updateWarehouseActiveStatus(client, { id, installationId, isActive, updatedBy }) {
  const now = new Date().toISOString();
 ...
 */
export async function hasActiveLocations(client, { warehouseId }) {
  const result = await client.query(
    `SELECT COUNT(*) as count FROM shared.warehouse_locations WHERE warehouse_id = $1 AND is_active = true`,
    [warehouseId],
  );
