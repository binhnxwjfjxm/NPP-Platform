...
export async function withAuditOutboxTransaction({ adapter, mutate }) {
  ...
  try {
    ...
    const result = await mutate(trackedClient, { buildAuditRecord, buildOutboxEvent, insertAuditRecord, insertOutboxEvent });
    if (result && (result.skipAudit === true || result.failed)) {
      await client.query('ROLLBACK');
      return result;
    }
    const replayWithoutWrites = result?.replayed === true && writeState.auditCount === 0 && writeState.outboxCount === 0;
