...
import {
  ...
  getDeliveryTripDispatchSummary,
} from '../services/logistics-trip-dispatch.js';
...
function sanitizedUnexpectedError(error, requestId) {
  return {
    event: 'logistics_dispatch_route_unexpected_error',
    requestId,
    name: typeof error?.name === 'string' ? error.name.slice(0, 80) : 'Error',
    code: typeof error?.code === 'string' ? error.code.slice(0, 80) : null,
    message: String(error?.message ?? 'Unknown logistics dispatch error')
...
function sendUnexpectedError(res, error, options) {
  ...
  sendError(
    res,
    apiError('LOGISTICS_DISPATCH_QUERY_FAILED', 'Dữ liệu bàn giao chuyến tạm thời không khả dụng', {}, true, 503),
    options.requestId,
    options.receivedAt,
  );
}
...
export async function handleLogisticsDispatchRoutes(req, res, options) {
  ...
  const match = pathname.match(/^\/api\/logistics\/trips\/([^/]+)\/dispatch$/);
  ...
  try {
    if (method === 'GET') {
      const requestContext = await authenticateAndAuthorize(
        ...
        res,
        options,
        options.PERMISSIONS.coreDeliveryTripRead,
      );
      if (!requestContext) return true;
      const result = await getDeliveryTripDispatchSummary(options.getPool(), {
        requestContext,
        tripId,
      });
    ...
    if (method === 'POST') {
      const requestContext = await authenticateAndAuthorize(
        ...
        res,
        options,
        options.PERMISSIONS.coreDeliveryTripDispatch,
      );
      if (!requestContext) return true;
      let payload;
      try {
      ...
      const result = await dispatchDeliveryTrip({
