...
import * as service from '../services/customer-receivable.js';
...
function statusFor(code) {
  ...
  if (code.includes('CONFLICT') || code.includes('MISMATCH') || code.includes('EXCEEDS')) return 409;
  return 400;
}
...
export async function handleCustomerReceivableCoreRoutes(req, res, options) {
  const url = new URL(`http://localhost${req.url}`);
  const pathname = url.pathname;
  if (pathname !== '/api/receivables' && !pathname.startsWith('/api/receivables/')) return false;
  ...
  try {
    if (pathname === '/api/receivables') {
      const result = await service.listReceivableDocuments(options.getPool(), {
        requestContext,
        customerId: url.searchParams.get('customerId'),
        warehouseId: url.searchParams.get('warehouseId'),
        status: url.searchParams.get('status'),
        sourceDocumentType: url.searchParams.get('sourceDocumentType'),
    ...
    if (pathname === '/api/receivables/balances') {
      const result = await service.listCustomerReceivableBalances(options.getPool(), {
        requestContext,
        customerId: url.searchParams.get('customerId'),
        search: url.searchParams.get('search'),
        limit: parseInteger(url.searchParams.get('limit'), 100, 1, 1000),
    ...
    const detail = pathname.match(/^\/api\/receivables\/([^/]+)$/);
    if (detail) {
      const result = await service.getReceivableDocument(options.getPool(), {
        requestContext,
        id: detail[1],
      });
      if (!result.ok) {
      ...
      sendSuccess(res, result.receivableDocument, options.requestId, options.receivedAt);
      return true;
    }
  } catch (error) {
    sendError(
      res,
      apiError(
        error?.code ?? 'RECEIVABLE_READ_FAILED',
        error?.publicMessage ?? 'Receivable read failed',
        {},
        false,
