...
async function executeMutation(req, res, options, {
  ...
  try {
    ...
    sendError(
      res,
      apiError(
        error?.code ?? 'SUPPLIER_PAYMENT_TRANSACTION_FAILED',
        'Supplier payment transaction failed',
        {},
        true,
...
export async function handleSupplierPaymentRoutes(req, res, options) {
  ...
  const isPayment = pathname === '/api/supplier-payments'
    || pathname.startsWith('/api/supplier-payments/');
  const isAllocation = pathname === '/api/payable-allocations'
    || pathname.startsWith('/api/payable-allocations/');
  ...
  try {
    if (pathname === '/api/supplier-payments' && method === 'GET') {
      const requestContext = await authenticateAndAuthorize(
        ...
        res,
        options,
        options.PERMISSIONS.coreSupplierPaymentRead,
      );
    ...
    if (pathname === '/api/supplier-payments/allocation-targets' && method === 'GET') {
      const requestContext = await authenticateAndAuthorize(
        ...
        res,
        options,
        options.PERMISSIONS.corePayableRead,
      );
      if (!requestContext) return true;
    ...
    if (pathname === '/api/supplier-payments' && method === 'POST') {
      const requestContext = await authenticateAndAuthorize(
        ...
        res,
        options,
        options.PERMISSIONS.coreSupplierPaymentCreate,
      );
      if (!requestContext) return true;
      ...
      await executeMutation(req, res, options, {
        ...
        statusCode: 201,
        action: 'create',
        resourceType: 'accounting.supplier_payment',
        eventType: 'accounting.supplier_payment.posted',
        mutate: (client, idempotencyKey) => service.createSupplierPayment(client, {
          requestContext,
          payload: input,
          idempotencyKey,
        }),
    ...
    if (paymentReverse && method === 'POST') {
      const requestContext = await authenticateAndAuthorize(
        ...
        res,
        options,
        options.PERMISSIONS.coreSupplierPaymentReverse,
      );
      if (!requestContext) return true;
      const input = await readPayload(req, res, options);
      if (input === null) return true;
      await executeMutation(req, res, options, {
        ...
        action: 'reverse',
        resourceType: 'accounting.supplier_payment',
        eventType: 'accounting.supplier_payment.reversed',
        mutate: (client) => service.reverseSupplierPayment(client, {
          requestContext,
          id: paymentReverse[1],
          payload: input,
        }),
    ...
    if (paymentDetail && method === 'GET') {
      ...
      if (!requestContext) return true;
      const result = await service.getSupplierPayment(options.getPool(), {
        requestContext,
        id: paymentDetail[1],
      });
      if (!result.ok) {
    ...
    if (pathname === '/api/payable-allocations' && method === 'POST') {
      const requestContext = await authenticateAndAuthorize(
        ...
        res,
        options,
        options.PERMISSIONS.corePayableAllocationCreate,
      );
      if (!requestContext) return true;
      const input = await readPayload(req, res, options);
      if (input === null) return true;
      await executeMutation(req, res, options, {
        ...
        statusCode: 201,
        action: 'create',
        resourceType: 'accounting.payable_allocation',
        eventType: 'accounting.payable_allocation.created',
        mutate: (client) => service.createPayableAllocation(client, {
          requestContext,
          payload: input,
        }),
      });
    ...
    if (allocationReverse && method === 'POST') {
      const requestContext = await authenticateAndAuthorize(
        ...
        res,
        options,
        options.PERMISSIONS.corePayableAllocationReverse,
      );
      if (!requestContext) return true;
      const input = await readPayload(req, res, options);
      if (input === null) return true;
      await executeMutation(req, res, options, {
        ...
        action: 'reverse',
        resourceType: 'accounting.payable_allocation',
        eventType: 'accounting.payable_allocation.reversed',
        mutate: (client) => service.reversePayableAllocation(client, {
          requestContext,
    ...
    sendError(
      res,
      apiError(
        error?.code ?? 'SUPPLIER_PAYMENT_REQUEST_FAILED',
        error?.publicMessage ?? 'Supplier payment request failed',
        {},
        false,
