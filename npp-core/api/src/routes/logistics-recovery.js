...
} from '../services/logistics-trip-recovery.js';
...
function statusFor(code) {
  ...
  if (code.includes('BLOCKED') || code.includes('CONFLICT') || code.includes('IDEMPOTENCY') || code.includes('NOT_ALLOWED') || code.includes('NOT_ACTIVE') || code.includes('ALREADY')) return 409;
  return 400;
}
...
async function scopedContext(req, res, options) {
  ...
  let context = options.createContext({ config: options.config, principal: auth.principal, requestId: options.requestId, receivedAt: options.receivedAt });
  if (!options.authorize(context, options.PERMISSIONS.coreDeliveryTripDispatch).ok) {
  ...
  if ((!context.scopes?.warehouseIds?.length) && context.roles?.includes('bootstrap')) {
    const warehouses = await warehouseRepository.listWarehousesForInstallation(options.getPool(), {
      installationId: context.installationId, active: true, limit: 10000, offset: 0,
    });
...
export async function handleLogisticsRecoveryRoutes(req, res, options) {
  const pathname = new URL(`http://localhost${req.url}`).pathname;
  const recoverMatch = pathname.match(/^\/api\/logistics\/trips\/([^/]+)\/recover-dispatch$/);
  const unassignMatch = pathname.match(/^\/api\/logistics\/trips\/([^/]+)\/recovery-unassign\/([^/]+)$/);
  if (!recoverMatch && !unassignMatch) return false;
  ...
  let idempotencyKey;
  try { idempotencyKey = normalizeIdempotencyKey(req.headers['idempotency-key']); }
  catch (error) {
  ...
  try {
    const result = recoverMatch
      ? await executeRecoverDeliveryTrip({ adapter: options.getPool(), requestContext, tripId: recoverMatch[1], idempotencyKey, payload })
      : await executeRecoveryUnassignDeliveryOrder({ adapter: options.getPool(), requestContext, tripId: unassignMatch[1], assignmentId: unassignMatch[2], idempotencyKey, payload });
    if (!result.ok) {
    ...
    console.error(JSON.stringify({ event: 'logistics_recovery_error', requestId: options.requestId, name: error?.name ?? 'Error' }));
    sendError(res, apiError('LOGISTICS_RECOVERY_FAILED', 'Trip recovery failed', {}, true, 503), options.requestId, options.receivedAt);
  }
  return true;
}
