...
test('10.2 tracking policy candidates are installation-scoped canonical inventory base SKUs and searchable by sibling SKU', async () => {
  const queries = [];
  const client = {
    async query(sql, params) {
      ...
      return {
        rows: [{
          ...
          product_active: true,
          has_policy: false,
          related_variant_search_text: 'SKU-001 Base SKU-001-THUNG Thùng',
        }],
      };
    },
  };
  ...
  assert.equal(result.candidates[0].related_variant_search_text.includes('SKU-001-THUNG'), true);
  assert.equal(queries.length, 1);
  assert.deepEqual(queries[0].params, ['installation-1', '%SKU-001-THUNG%', 100, 0]);
  ...
  assert.match(queries[0].sql, /related_variant_search_text/);
  assert.match(queries[0].sql, /FROM shared\.product_variants related_search/);
  assert.match(queries[0].sql, /related_search\.sku ILIKE \$2/);
});
