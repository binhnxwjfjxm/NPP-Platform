...
async function seed(pool, installationId) {
  ...
  await pool.query(
    `INSERT INTO shared.warehouses(id,installation_id,branch_id,code,name,warehouse_type,is_active,created_by,updated_by)
     VALUES($1,$2,$3,$4,$5,'main',true,$6,$6),
           ($7,$2,$3,$8,$9,'main',true,$6,$6)`,
    [ids.warehouseId, installationId, ids.branchId, `WH-${suffix}`, `Kho ${suffix}`, actor,
      ids.otherWarehouseId, `WHX-${suffix}`, `Kho khác ${suffix}`],
  );
  await pool.query(
  ...
  await pool.query(
    `INSERT INTO shared.document_number_series(
       ...
       sequence_width,start_counter,timezone_name,is_active,created_by,updated_by
     ) VALUES($1,$2,'SUPPLIER_PAYMENT','SUPPLIER_PAYMENT','Phiếu thanh toán NCC','SP-',
       '{PREFIX}{YYYY}{MM}-{SEQ}','MONTHLY',6,1,'Asia/Ho_Chi_Minh',true,$3,$3)`,
    [ids.seriesId, installationId, actor],
  );
  await pool.query(
    ...
    [ids.debitId, installationId, ids.supplierId, ids.warehouseId, ids.debitSourceId, `GR-${suffix}`, actor],
  );
  await pool.query(
    ...
    [randomUUID(), installationId, ids.debitId, ids.supplierId, ids.debitSourceId, `GR-${suffix}`, actor, `seed-${suffix}`],
  );
  return ids;
}

async function createPayment(baseUrl, config, fixture, key, amount = '60000') {
  const response = await fetch(`${baseUrl}/api/supplier-payments`, {
    method: 'POST',
    ...
    body: JSON.stringify({
      ...
      amount,
      externalReference: `BANK-${key}`,
      note: 'Thanh toán kiểm thử Phase 5.6',
    }),
  });
  return response;
}

test('Phase 5.6 posts, allocates and reverses supplier payments without changing allocation balance', async () => {
  ...
  try {
    ...
    let response = await createPayment(baseUrl, config, fixture, paymentKey);
    ...
    const allocationKey = `allocation-${randomUUID()}`;
    response = await fetch(`${baseUrl}/api/payable-allocations`, {
      method: 'POST',
      headers: headers(config, allocationKey),
      body: JSON.stringify({
        sourcePayableDocumentId: payment.id,
        targetPayableDocumentId: fixture.debitId,
        amount: '50000',
    ...
    assert.equal(byId.get(fixture.debitId).allocated_amount, '50000.000000');
    assert.equal(byId.get(fixture.debitId).remaining_amount, '50000.000000');

    response = await fetch(`${baseUrl}/api/payable-allocations`, {
      method: 'POST',
      headers: headers(config, `over-${randomUUID()}`),
      body: JSON.stringify({
        sourcePayableDocumentId: payment.id,
        targetPayableDocumentId: fixture.debitId,
        amount: '11000',
    ...
    response = await fetch(`${baseUrl}/api/payable-allocations/${allocation.id}/reverse`, {
      method: 'POST',
      headers: headers(config, `allocation-reverse-${randomUUID()}`),
    ...
    response = await fetch(`${baseUrl}/api/supplier-payments/${payment.id}/reverse`, {
      method: 'POST',
      headers: headers(config, `payment-reverse-${randomUUID()}`),
    ...
    response = await fetch(`${baseUrl}/api/payable-allocations`, {
      method: 'POST',
      headers: headers(config, `credit-allocation-${randomUUID()}`),
      body: JSON.stringify({
        sourcePayableDocumentId: creditId,
        targetPayableDocumentId: fixture.debitId,
        amount: '30000',
        allocationDate: '2026-07-30',
      }),
    ...
    await assert.rejects(
      pool.query(
        `UPDATE accounting.payable_allocations SET amount=1 WHERE installation_id=$1 AND id=$2`,
        [config.installationId, creditAllocation.id],
      ),
      /payable_allocation_history_is_append_only/,
    );

    const paymentA = await responseData(await createPayment(baseUrl, config, fixture, `concurrent-a-${randomUUID()}`, '50000'));
    const paymentB = await responseData(await createPayment(baseUrl, config, fixture, `concurrent-b-${randomUUID()}`, '50000'));
    const allocate = (sourceId, key) => fetch(`${baseUrl}/api/payable-allocations`, {
      method: 'POST',
      headers: headers(config, key),
      body: JSON.stringify({
        sourcePayableDocumentId: sourceId,
        targetPayableDocumentId: fixture.debitId,
        amount: '50000',
        allocationDate: '2026-07-30',
      }),
    ...
    const concurrent = await Promise.all([
      allocate(paymentA.id, `concurrent-allocate-a-${randomUUID()}`),
      allocate(paymentB.id, `concurrent-allocate-b-${randomUUID()}`),
    ]);
