...
test('Supplier return lifecycle keeps trusted snapshots and blocks goods receipt reversal while active', async () => {
  ...
  try {
    ...
    const overReturnResponse = await fetch(`${baseUrl}/api/supplier-returns`, {
      method: 'POST',
    ...
    const staleApprove = await fetch(`${baseUrl}/api/supplier-returns/${draftReturn.id}/approve`, {
      method: 'POST',
    ...
    const postKey = `sr-post-${randomUUID()}`;
    const postPayload = {
    ...
    async function createApprovedConcurrentReturn(label) {
