...
import { GOODS_RECEIPT_PERMISSION_KEYS } from './goods-receipt-types';
...
function requiredServerValue(name: 'CORE_API_INTERNAL_URL' | 'CORE_API_SERVER_TOKEN'): string {
  ...
  if (!value) throw new Error('goods_receipt_context_not_configured');
  return value;
}
...
function coreApiBaseUrl(): string {
  ...
  if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw new Error('goods_receipt_context_not_configured');
  }
...
export async function loadGoodsReceiptPermissionKeys(requestId?: string | null): Promise<string[]> {
  ...
  try {
    ...
    if (!response.ok || !payload?.data) throw new Error('goods_receipt_context_unavailable');
    ...
    return Object.values(GOODS_RECEIPT_PERMISSION_KEYS).filter((key) => granted.has(key));
  } finally {
