...
export function resolveOrganizationRequestId(value: string | null | undefined): string {
  const candidate = String(value ?? '').trim();
  return REQUEST_ID_PATTERN.test(candidate) ? candidate : `web_${randomUUID()}`;
}

export function normalizeOrganizationGatewayError(error: unknown): OrganizationGatewayError {
  if (error instanceof OrganizationGatewayError) return error;
  return new OrganizationGatewayError(
...
function organizationPath(resource: OrganizationResource, id?: string): string {
