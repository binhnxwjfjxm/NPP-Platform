...
function normalizeDiscountMode(value) {
  ...
  if (['%', 'PERCENT', '% TIỀN HÀNG', '% TIEN HANG', 'PHẦN TRĂM', 'PHAN TRAM'].includes(normalized)) return PURCHASE_ORDER_DISCOUNT_MODES.percent;
  if (['PER_UNIT', 'UNIT', 'GIAM_MOI_DON_VI', 'GIẢM MỖI ĐƠN VỊ', 'GIAM MOI DON VI'].includes(normalized)) return PURCHASE_ORDER_DISCOUNT_MODES.perUnit;
  if (['', 'TOTAL_AMOUNT', 'TOTAL', 'AMOUNT', 'GIẢM TỔNG DÒNG', 'GIAM TONG DONG', 'TỔNG GIẢM DÒNG', 'TONG GIAM DONG'].includes(normalized)) return PURCHASE_ORDER_DISCOUNT_MODES.totalAmount;
  return null;
}
...
export function calculatePurchaseOrderLineFinancials(line) {
  ...
  const hasTaxRate = line.taxRate !== undefined && line.taxRate !== null && String(line.taxRate).trim() !== '';
