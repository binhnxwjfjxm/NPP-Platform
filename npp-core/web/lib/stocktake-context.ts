...
import { STOCKTAKE_PERMISSION_KEYS } from './stocktake-types';
...
function requiredServerValue(name: 'CORE_API_INTERNAL_URL' | 'CORE_API_SERVER_TOKEN'): string {
  ...
  if (!value) throw new Error('stocktake_context_not_configured');
  return value;
}
...
function coreApiBaseUrl(): string {
  ...
  if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw new Error('stocktake_context_not_configured');
  }
...
export async function loadStocktakePermissionKeys(requestId?: string | null): Promise<string[]> {
  ...
  try {
    ...
    if (!response.ok || !payload?.data) throw new Error('stocktake_context_unavailable');
    ...
    return Object.values(STOCKTAKE_PERMISSION_KEYS).filter((key) => granted.has(key));
  } finally {
