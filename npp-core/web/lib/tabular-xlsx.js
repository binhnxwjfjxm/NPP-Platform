...
function normalizeTable({ sheetName = 'Dữ liệu', headers, rows }, limits) {
  if (!Array.isArray(headers) || headers.length < 1 || headers.length > limits.maxColumns) throw new Error('XLSX_COLUMN_LIMIT_EXCEEDED');
  if (!Array.isArray(rows) || rows.length + 1 > limits.maxRows) throw new Error('XLSX_ROW_LIMIT_EXCEEDED');
  const safeHeaders = headers.map((value) => String(value ?? '').trim());
  if (safeHeaders.some((value) => !value)) throw new Error('XLSX_HEADER_INVALID');
...
export function createTabularXlsx(input, limits = TABULAR_XLSX_LIMITS) {
  ...
  const emptyRow = table.rows.length === 0 ? `<row r="2">${table.headers.map((_, index) => inlineCell(`${columnName(index)}2`, '')).join('')}</row>` : '';
  ...
  const tableColumns = table.headers.map((header, index) => `<tableColumn id="${index + 1}" name="${xmlEscape(header)}"/>`).join('');
  return zipStored([
    ...
    ['xl/workbook.xml', `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="${xmlEscape(table.sheetName)}" sheetId="1" r:id="rId1"/></sheets></workbook>`],
    ...
    ['xl/tables/table1.xml', `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><table xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" id="1" name="DataExchange" displayName="DataExchange" ref="${tableRef}" totalsRowShown="0"><autoFilter ref="${tableRef}"/><tableColumns count="${table.headers.length}">${tableColumns}</tableColumns><tableStyleInfo name="TableStyleMedium2" showFirstColumn="0" showLastColumn="0" showRowStripes="1" showColumnStripes="0"/></table>`],
  ]);
}
