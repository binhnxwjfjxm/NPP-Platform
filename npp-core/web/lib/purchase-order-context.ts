...
import { PURCHASE_ORDER_PERMISSION_KEYS } from './purchase-order-types';
...
function requiredServerValue(name: 'CORE_API_INTERNAL_URL' | 'CORE_API_SERVER_TOKEN'): string {
  ...
  if (!value) throw new Error('purchase_order_context_not_configured');
  return value;
}
...
function coreApiBaseUrl(): string {
  ...
  if (!['http:', 'https:'].includes(parsed.protocol) || parsed.username || parsed.password) {
    throw new Error('purchase_order_context_not_configured');
  }
...
export async function loadPurchaseOrderPermissionKeys(requestId?: string | null): Promise<string[]> {
  ...
  try {
    ...
    if (!response.ok || !payload?.data) throw new Error('purchase_order_context_unavailable');
    ...
    return Object.values(PURCHASE_ORDER_PERMISSION_KEYS).filter((key) => granted.has(key));
  } finally {
