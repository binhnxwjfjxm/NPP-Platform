...
const apiEnvironment = {
  NODE_ENV: 'test', HOST: '127.0.0.1', PORT: '3004', INSTALLATION_ID: 'e2e-installation',
  DATABASE_URL: databaseUrl,
  DATABASE_SSL_MODE: process.env.E2E_DATABASE_SSL_MODE?.trim() || 'disable',
  BACKEND_API_TOKEN: backendToken, CORE_BOOTSTRAP_ACTOR_ID: 'bootstrap:e2e',
  CORS_ORIGINS: 'http://127.0.0.1:3003,http://127.0.0.1:3005',
  R2_ENABLED: 'false', R2_CONTRACT_ROUTE_ENABLED: 'false',
};
const commonWebEnvironment = {
  NODE_ENV: 'test', HOST: '127.0.0.1', NEXT_PUBLIC_CORE_API_URL: 'http://127.0.0.1:3004',
  NEXT_PUBLIC_APP_NAME: 'NPP Core', CORE_API_INTERNAL_URL: 'http://127.0.0.1:3004',
  CORE_API_SERVER_TOKEN: backendToken, CORE_WEB_ADMIN_USERNAME: webAdminUsername,
  CORE_WEB_ADMIN_PASSWORD: webAdminPassword, FOUNDATION_R2_TEST_ENABLED: 'false',
};
const authenticatedBrowser = {
  ...devices['Desktop Chrome'], baseURL: 'http://127.0.0.1:3003',
  httpCredentials: { username: webAdminUsername, password: webAdminPassword },
};

export default defineConfig({
  testDir: './e2e', fullyParallel: false, forbidOnly: Boolean(process.env.CI),
  ...
  projects: [
    ...
    { name: 'purchasing', testMatch: /(purchase-orders|goods-receipts|supplier-returns)\.spec\.ts/, dependencies: ['organization', 'suppliers', 'catalog'], use: authenticatedBrowser },
    { name: 'sales-order-commercial', testMatch: /sales-orders-(commercial|price-recovery)\.spec\.ts/, dependencies: ['organization'], use: authenticatedBrowser },
  ...
  webServer: [
    { command: 'npm --prefix ../api run dev', url: 'http://127.0.0.1:3004/health/live', reuseExistingServer: false, env: apiEnvironment, timeout: 120_000 },
    { command: 'npm run dev', url: 'http://127.0.0.1:3003', reuseExistingServer: false, env: { ...commonWebEnvironment, PORT: '3003', FOUNDATION_TEST_UI_ENABLED: 'false' }, timeout: 120_000 },
    { command: 'npx next dev -p 3005 -H 127.0.0.1', url: 'http://127.0.0.1:3005', reuseExistingServer: false, env: { ...commonWebEnvironment, PORT: '3005', FOUNDATION_TEST_UI_ENABLED: 'true' }, timeout: 120_000 },
  ],
});
