...
async function requestJson<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok || payload.data === undefined) {
    ...
    const normalized = path.includes('/sku-search')
...
function optionLabel(option: PurchaseOrderSkuSearchOption) {
  return `${option.productCode} · ${option.productName} / ${option.sku} · ${option.variantName}`;
}

export default function PurchaseOrderEditorV2({
  mode,
  purchaseOrder,
  suppliers,
  warehouses,
  ...
  async function loadSkuPage(term: string, offset: number, target: 'quick' | 'browse', append = false, signal?: AbortSignal) {
    const query = new URLSearchParams({ search: term.trim(), limit: String(SEARCH_PAGE_SIZE), offset: String(offset) });
    const results = await requestJson<PurchaseOrderSkuSearchOption[]>(`/api/purchase-orders/sku-search?${query.toString()}`, { signal });
    if (target === 'quick') {
      setQuickResults((current) => append ? [...new Map([...current, ...results].map((option) => [option.id, option])).values()] : results);
      setQuickHasMore(results.length === SEARCH_PAGE_SIZE);
    } else {
      setBrowseResults((current) => append ? [...new Map([...current, ...results].map((option) => [option.id, option])).values()] : results);
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(async () => {
      ...
      try {
        await loadSkuPage(term, 0, 'quick', false, controller.signal);
      } catch (searchError) {
  ...
  useEffect(() => {
    if (entryMode !== 'browse' || browseResults.length > 0) return;
    void searchBrowse();
  }, [entryMode]);
  ...
  async function searchBrowse() {
    ...
    try {
      await loadSkuPage(browseTerm, 0, 'browse');
    } catch (searchError) {
      setBrowseResults([]);
  ...
  async function loadMore(target: 'quick' | 'browse') {
    const isQuick = target === 'quick';
    const term = isQuick ? quickTerm : browseTerm;
    const current = isQuick ? quickResults : browseResults;
    if ((isQuick && !quickHasMore) || (!isQuick && !browseHasMore)) return;
    if (isQuick) setQuickLoading(true); else setBrowseLoading(true);
    try {
      await loadSkuPage(term, current.length, target, true);
    } catch (searchError) {
      setError(searchError instanceof Error ? searchError.message : 'Không tải thêm được SKU.');
    } finally {
      if (isQuick) setQuickLoading(false); else setBrowseLoading(false);
    }
  }
  ...
  function addBrowseSelection() {
    const additions = browseResults.filter((option) => selectedBrowseIds.has(option.id) && option.eligibility.selectable);
    let added = 0;
    ...
    setSelectedBrowseIds(new Set());
    if (added === 0) setError('Chưa có SKU hợp lệ mới để thêm.');
  }
  ...
  function handleQuickKeyDown(event: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (event.key === 'ArrowDown') {
      ...
      setSelectedSku(option ?? null);
      if (option?.eligibility.selectable) addSku(option);
  ...
  function addBulkRows() {
    const additions = bulkPreview
      ...
      .map((row): EditorLine => {
        ...
        return {
          key: crypto.randomUUID(), variantId: option.id, sku: option.sku, name: option.variantName,
          unitCode: option.unitCode ?? '', conversionToBase: cleanDecimal(option.conversionToBase, '1'),
          quantity: toApiDecimal(row.quantity, '1'), unitPrice: toApiDecimal(row.unitPrice, '0'),
          discountMode: row.discountMode, discountValue: toApiDecimal(row.discountValue, '0'), discountAmount: '0',
          taxRate: toApiDecimal(row.taxRate, '0'), taxAmount: '0', note: row.note,
        };
      });
  ...
  return (
    <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && !busy) onClose(); }}>
      <section className={localStyles.dialog} role="dialog" aria-modal="true" aria-labelledby="purchase-order-editor-title">
          ...
          <div className={localStyles.dialogBody}>
            ...
            <section className={localStyles.section} aria-labelledby="po-header-title">
              ...
              <div className={localStyles.headerGrid}>
                ...
                <label className={localStyles.fullWidth}>Ghi chú đơn hàng<input value={note} maxLength={4000} onChange={(event) => { changed(); setNote(event.target.value); }} /></label>
              </div>
            </section>
            ...
            <section className={localStyles.section} aria-labelledby="po-entry-title">
              ...
              <div className={localStyles.modeTabs} role="tablist" aria-label="Cách thêm dòng đặt hàng">
                {([['quick', 'Tìm nhanh'], ['browse', 'Chọn từ danh mục'], ['bulk', 'Nhập nhiều dòng']] as const).map(([value, label]) => (
                  <button key={value} type="button" role="tab" aria-selected={entryMode === value} className={entryMode === value ? localStyles.modeTabActive : localStyles.modeTab} onClick={() => { setEntryMode(value); setError(null); }}>{label}</button>
                ))}
              </div>

              {entryMode !== 'bulk' ? (
                <div className={localStyles.filterBar}>
                  <label>Trạng thái SKU<select value={eligibilityFilter} onChange={(event) => setEligibilityFilter(event.target.value as PurchaseOrderSkuFilter)}><option value="eligible">Có thể mua</option><option value="setup">Cần thiết lập</option><option value="all">Tất cả</option></select></label>
                  <Link href="/products" className={localStyles.catalogLink}>Mở thiết lập sản phẩm</Link>
                </div>
              ) : null}

              {entryMode === 'quick' ? (
                <div className={localStyles.modePanel}>
                  <div className={localStyles.quickRow}>
                    <label>Từ khóa sản phẩm hoặc SKU
                      <div className={localStyles.searchInputWrap}>
                        <input role="combobox" aria-expanded={filteredQuickResults.length > 0} aria-controls="po-quick-results" placeholder="Mã sản phẩm, tên, SKU hoặc mã vạch" value={quickTerm} onKeyDown={handleQuickKeyDown} onChange={(event) => setQuickTerm(event.target.value)} />
                        {quickTerm ? <button type="button" onClick={() => { setQuickTerm(''); setQuickResults([]); setError(null); }}>Xóa</button> : null}
                      </div>
                    </label>
                    <p className={localStyles.hint}>{quickTerm.trim().length < MIN_PURCHASE_ORDER_SKU_SEARCH_LENGTH ? 'Nhập ít nhất 2 ký tự để tìm.' : `${filteredQuickResults.length} kết quả đang hiển thị.`}</p>
                  </div>
                  <div id="po-quick-results" role="listbox" className={localStyles.resultList}>
                    ...
                    {filteredQuickResults.map((option, index) => (
                      <button key={option.id} type="button" role="option" aria-selected={selectedSku?.id === option.id || activeQuickIndex === index} className={`${localStyles.resultCard} ${activeQuickIndex === index ? localStyles.resultCardActive : ''}`} onMouseEnter={() => setActiveQuickIndex(index)} onClick={() => { setSelectedSku(option); if (option.eligibility.selectable) addSku(option); else setError(option.eligibility.message); }}>
                        <span className={localStyles.resultIdentity}><strong>{option.productCode} — {option.productName}</strong><span>{option.sku} — {option.variantName}</span></span>
                        <span className={localStyles.resultMeta}>{option.unitCode || 'Chưa có đơn vị'} · Quy đổi {option.conversionToBase || '—'}</span>
                        <span className={option.eligibility.selectable ? localStyles.eligible : localStyles.needsSetup}>{option.eligibility.message}</span>
                      </button>
                    ))}
                    {quickHasMore ? <button type="button" className={localStyles.loadMore} onClick={() => void loadMore('quick')} disabled={quickLoading}>Tải thêm kết quả</button> : null}
                  </div>
                </div>
              ) : null}
              ...
              {entryMode === 'browse' ? (
                <div className={localStyles.modePanel}>
                  <div className={localStyles.browseFilters}>
                    <label>Lọc sản phẩm<input value={browseTerm} placeholder="Mã hoặc tên sản phẩm" onChange={(event) => setBrowseTerm(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void searchBrowse(); } }} /></label>
                    <label>Đơn vị<select value={browseUnit} onChange={(event) => setBrowseUnit(event.target.value)}><option value="all">Tất cả đơn vị</option>{browseUnits.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</select></label>
                    <button type="button" className={styles.secondaryButton} onClick={() => void searchBrowse()} disabled={browseLoading}>{browseLoading ? 'Đang lọc…' : 'Lọc danh mục'}</button>
                  </div>
                  ...
                  <div className={localStyles.productGroups}>
                    {browseLoading && browseResults.length === 0 ? <p className={localStyles.empty}>Đang tải danh mục…</p> : null}
                    {!browseLoading && browseGroups.length === 0 ? <p className={localStyles.empty}>Không có sản phẩm phù hợp.</p> : null}
                    {browseGroups.map((group) => (
                      <details key={group.productId} className={localStyles.productGroup}>
                        <summary><span><strong>{group.productCode}</strong> — {group.productName}</span><span>{group.options.length} SKU</span></summary>
                        <div className={localStyles.productSkuList}>
                          {group.options.map((option) => {
                            const checked = selectedBrowseIds.has(option.id);
                            return <label key={option.id} className={localStyles.browseSku}>
                              <input type="checkbox" checked={checked} disabled={!option.eligibility.selectable || lines.some((line) => line.variantId === option.id)} onChange={(event) => setSelectedBrowseIds((current) => { const next = new Set(current); if (event.target.checked) next.add(option.id); else next.delete(option.id); return next; })} />
                              <span className={localStyles.resultIdentity}><strong>{option.sku} — {option.variantName}</strong><span>{option.unitCode || 'Chưa có đơn vị'} · Quy đổi {option.conversionToBase || '—'}</span></span>
                              <span className={option.eligibility.selectable ? localStyles.eligible : localStyles.needsSetup}>{option.eligibility.message}</span>
                            </label>;
                          })}
                        </div>
                  ...
                  {browseHasMore ? <button type="button" className={localStyles.loadMore} onClick={() => void loadMore('browse')} disabled={browseLoading}>Tải thêm sản phẩm/SKU</button> : null}
                </div>
              ) : null}
            ...
            <section className={localStyles.section} aria-labelledby="po-lines-title">
              ...
              <div className={localStyles.lineList}>
                {lines.length === 0 ? <p className={localStyles.empty}>Chưa có SKU trong đơn đặt hàng.</p> : lines.map((line, index) => (
                  <article key={line.key} className={localStyles.lineCard}>
                    <div className={localStyles.lineHeading}><div><strong>{line.sku}</strong><span>{line.name}</span></div><button type="button" className={styles.secondaryButton} onClick={() => { changed(); setLines((current) => current.filter((item) => item.key !== line.key)); }}>Xóa dòng</button></div>
                    <div className={localStyles.lineFields}>
                      <label>Số lượng<input value={line.quantity} inputMode="decimal" onBlur={() => formatLineDecimal(line.key, 'quantity', '1')} onChange={(event) => updateDecimalLine(line.key, 'quantity', event.target.value)} /></label>
                      <label>Đơn vị<input value={line.unitCode} readOnly /></label>
                      <label>Quy đổi<input value={line.conversionToBase} readOnly /></label>
                      <label>Đơn giá<input value={line.unitPrice} inputMode="decimal" onBlur={() => formatLineDecimal(line.key, 'unitPrice', '0')} onChange={(event) => updateDecimalLine(line.key, 'unitPrice', event.target.value)} /></label>
                      <label className={localStyles.discountMode}>Kiểu chiết khấu<select value={line.discountMode ?? 'TOTAL_AMOUNT'} onChange={(event) => updateLine(line.key, 'discountMode', event.target.value as PurchaseOrderDiscountMode)}><option value="PERCENT">% tiền hàng</option><option value="PER_UNIT">Giảm mỗi đơn vị</option><option value="TOTAL_AMOUNT">Giảm tổng dòng</option></select></label>
                      <label>Giá trị chiết khấu<input value={line.discountValue ?? '0'} inputMode="decimal" onBlur={() => formatLineDecimal(line.key, 'discountValue', '0')} onChange={(event) => updateDecimalLine(line.key, 'discountValue', event.target.value)} /></label>
                      <label>Thuế suất %<input value={line.taxRate ?? ''} inputMode="decimal" aria-label={`Thuế suất dòng ${index + 1}`} onBlur={() => formatLineDecimal(line.key, 'taxRate', '')} onChange={(event) => updateDecimalLine(line.key, 'taxRate', event.target.value)} /></label>
                      <div className={localStyles.amountField}><span>Thành tiền</span><strong>{formatPurchaseOrderAmount(totals.lineTotals[index], purchaseOrder?.currency || 'VND')}</strong></div>
            ...
            <section className={localStyles.totals} aria-label="Tổng tiền đơn đặt hàng">
              <div><span>Tiền hàng</span><strong>{formatPurchaseOrderAmount(totals.subtotal, purchaseOrder?.currency || 'VND')}</strong></div>
              <div><span>Chiết khấu</span><strong>{formatPurchaseOrderAmount(totals.discountTotal, purchaseOrder?.currency || 'VND')}</strong></div>
              <div><span>Thuế</span><strong>{formatPurchaseOrderAmount(totals.taxTotal, purchaseOrder?.currency || 'VND')}</strong></div>
              <div><span>Tổng cộng</span><strong>{formatPurchaseOrderAmount(totals.total, purchaseOrder?.currency || 'VND')}</strong></div>
            </section>
          </div>

          <footer className={localStyles.dialogFooter}><button type="button" className={styles.secondaryButton} onClick={onClose} disabled={busy}>Hủy thao tác</button><button type="submit" className={styles.primaryButton} disabled={busy} data-testid="purchase-order-save">{busy ? 'Đang lưu…' : mode === 'create' ? 'Lưu đơn nháp' : 'Lưu thay đổi'}</button></footer>
        </form>
      </section>
    </div>
  );
