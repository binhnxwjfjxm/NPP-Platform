          ...
          <section className={`${styles.modal} ${localStyles.detailModal}`} role="dialog" aria-modal="true" aria-labelledby="purchase-order-detail-title">
            ...
            <div className={localStyles.linesWrap}>
              <table className={localStyles.linesTable}>
                <thead><tr><th>SKU</th><th>Số lượng</th><th>Đơn vị</th><th>Quy đổi</th><th>Đơn giá</th><th>Chiết khấu</th><th>Thuế</th><th>Thành tiền</th></tr></thead>
                <tbody>
                  {(selectedPurchaseOrder.lines ?? []).map((line) => (
                    <tr key={line.id}>
                      ...
                      <td>{formatDecimalString(line.quantity)}</td><td>{line.unitCode}</td><td>{formatDecimalString(line.conversionToBase)}</td>
