          ...
          <div className={styles.modalBackdrop} role="presentation" onClick={() => setEditing(false)}>
            <div className={styles.modal} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
              ...
              <form className={styles.form} onSubmit={(event) => void submit(event)}>
                ...
                <label>Cho phép đi trễ (phút)<input type="number" min={0} max={240} value={draft.lateGraceMinutes} onChange={(e) => setDraft((c) => ({ ...c, lateGraceMinutes: e.target.value }))} /></label>
                <label>Cho phép về sớm (phút)<input type="number" min={0} max={240} value={draft.earlyLeaveGraceMinutes} onChange={(e) => setDraft((c) => ({ ...c, earlyLeaveGraceMinutes: e.target.value }))} /></label>
