            ...
            <section className={joinClasses(styles.modal, customerStyles.modalWide)} role="dialog" aria-modal="true" aria-label="Biểu mẫu khách hàng">
              ...
              <form className={styles.form} onSubmit={submitCustomer}>
                <div className={customerStyles.formGrid}>
                  <label>Mã khách hàng<input data-testid="customer-code-input" value={customerDraft.code} onChange={(event) => setCustomerDraft((value) => ({ ...value, code: event.currentTarget.value }))} disabled={customerEditor.mode === 'edit'} required /></label>
                  <label>Tên khách hàng<input data-testid="customer-name-input" value={customerDraft.name} onChange={(event) => setCustomerDraft((value) => ({ ...value, name: event.currentTarget.value }))} required /></label>
                  <label>Nhóm khách hàng<select value={customerDraft.groupId} onChange={(event) => setCustomerDraft((value) => ({ ...value, groupId: event.currentTarget.value }))}><option value="">Không phân nhóm</option>{groups.filter((group) => group.is_active || group.id === customerDraft.groupId).map((group) => <option key={group.id} value={group.id}>{group.code} · {group.name}</option>)}</select></label>
                  <label>Nhân sự phụ trách<select value={customerDraft.responsibleEmployeeId} onChange={(event) => setCustomerDraft((value) => ({ ...value, responsibleEmployeeId: event.currentTarget.value }))}><option value="">Chưa giao phụ trách</option>{employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.code} · {employee.full_name}</option>)}</select></label>
                  <label>Điện thoại<input data-testid="customer-phone-input" value={customerDraft.phone} onChange={(event) => setCustomerDraft((value) => ({ ...value, phone: event.currentTarget.value }))} /></label>
                  <label>Email<input data-testid="customer-email-input" type="email" value={customerDraft.email} onChange={(event) => setCustomerDraft((value) => ({ ...value, email: event.currentTarget.value }))} /></label>
                  <label>Mã số thuế<input value={customerDraft.taxCode} onChange={(event) => setCustomerDraft((value) => ({ ...value, taxCode: event.currentTarget.value }))} /></label>
                  <label>Thời hạn thanh toán (ngày)<input type="number" min="0" max="3650" value={customerDraft.paymentTermsDays} onChange={(event) => setCustomerDraft((value) => ({ ...value, paymentTermsDays: event.currentTarget.value }))} /></label>
                  <label>Hạn mức tín dụng<input type="number" min="0" step="0.01" value={customerDraft.creditLimit} onChange={(event) => setCustomerDraft((value) => ({ ...value, creditLimit: event.currentTarget.value }))} /></label>
                  <label className={customerStyles.fullWidth}>Ghi chú<textarea value={customerDraft.notes} onChange={(event) => setCustomerDraft((value) => ({ ...value, notes: event.currentTarget.value }))} /></label>
                </div>
          ...
          <div className={styles.modalBackdrop} role="presentation">
            <section className={styles.modal} role="dialog" aria-modal="true" aria-label="Biểu mẫu nhóm khách hàng">
              ...
              <form className={styles.form} onSubmit={submitGroup}>
                <div className={customerStyles.formGrid}>
                  <label>Mã nhóm<input data-testid="customer-group-code-input" value={groupDraft.code} onChange={(event) => setGroupDraft((value) => ({ ...value, code: event.currentTarget.value }))} disabled={groupEditor.mode === 'edit'} required /></label>
                  <label>Tên nhóm<input data-testid="customer-group-name-input" value={groupDraft.name} onChange={(event) => setGroupDraft((value) => ({ ...value, name: event.currentTarget.value }))} required /></label>
                  <label className={customerStyles.fullWidth}>Mô tả<textarea value={groupDraft.description} onChange={(event) => setGroupDraft((value) => ({ ...value, description: event.currentTarget.value }))} /></label>
                </div>
                ...
                <div>
                  {addressEditor ? (
                    <form className={styles.form} onSubmit={submitAddress}>
                      <div className={customerStyles.formGrid}>
                        <label>Nhãn địa chỉ<input data-testid="customer-address-label-input" value={addressDraft.label} onChange={(event) => setAddressDraft((value) => ({ ...value, label: event.currentTarget.value }))} required /></label>
                        <label>Người nhận<input value={addressDraft.recipientName} onChange={(event) => setAddressDraft((value) => ({ ...value, recipientName: event.currentTarget.value }))} /></label>
                        <label>Điện thoại<input value={addressDraft.phone} onChange={(event) => setAddressDraft((value) => ({ ...value, phone: event.currentTarget.value }))} /></label>
                        <label>Quốc gia<input value={addressDraft.countryCode} maxLength={2} onChange={(event) => setAddressDraft((value) => ({ ...value, countryCode: event.currentTarget.value }))} /></label>
                        <label className={customerStyles.fullWidth}>Địa chỉ dòng 1<input data-testid="customer-address-line1-input" value={addressDraft.addressLine1} onChange={(event) => setAddressDraft((value) => ({ ...value, addressLine1: event.currentTarget.value }))} required /></label>
                        <label className={customerStyles.fullWidth}>Địa chỉ dòng 2<input value={addressDraft.addressLine2} onChange={(event) => setAddressDraft((value) => ({ ...value, addressLine2: event.currentTarget.value }))} /></label>
                        <label>Phường/xã<input value={addressDraft.ward} onChange={(event) => setAddressDraft((value) => ({ ...value, ward: event.currentTarget.value }))} /></label>
                        <label>Quận/huyện<input value={addressDraft.district} onChange={(event) => setAddressDraft((value) => ({ ...value, district: event.currentTarget.value }))} /></label>
                        <label>Tỉnh/thành<input value={addressDraft.province} onChange={(event) => setAddressDraft((value) => ({ ...value, province: event.currentTarget.value }))} /></label>
                        <label>Mã bưu chính<input value={addressDraft.postalCode} onChange={(event) => setAddressDraft((value) => ({ ...value, postalCode: event.currentTarget.value }))} /></label>
                        <label className={joinClasses(customerStyles.inlineCheck, customerStyles.fullWidth)}><input type="checkbox" checked={addressDraft.isDefault} onChange={(event) => setAddressDraft((value) => ({ ...value, isDefault: event.currentTarget.checked }))} />Đặt làm địa chỉ mặc định</label>
                      </div>
