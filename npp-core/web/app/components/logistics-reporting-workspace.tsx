...
import { LogisticsReportingExportActions } from './reporting-lot3-export-actions';
import {
  WorkspaceTabPanel,
  WorkspaceTabs,
...
type LogisticsReportTab = 'drivers' | 'vehicles' | 'results' | 'trips' | 'exceptions';

const LOGISTICS_TABS: readonly WorkspaceTabOption<LogisticsReportTab>[] = Object.freeze([
  { id: 'drivers', label: 'Tài xế' },
  { id: 'vehicles', label: 'Phương tiện' },
  { id: 'results', label: 'Kết quả giao' },
  { id: 'trips', label: 'Chuyến gần nhất' },
  { id: 'exceptions', label: 'Ngoại lệ' },
]);

const LOGISTICS_TAB_PREFIX = 'logistics-reporting';
...
function deliveryResultLabel(value: string) {
  return ({
...
function tripStatusLabel(value: string) {
  return ({
    ...
    closed: 'Đã đóng',
    cancelled: 'Đã hủy',
  } as Record<string, string>)[value.toLowerCase()] ?? 'Trạng thái khác';
}
...
export function LogisticsReportingWorkspace() {
  ...
  const [activeTab, setActiveTab] = useState<LogisticsReportTab>('drivers');
  ...
  const actions = (
    <div className={styles.headerActions}>
      {report ? <LogisticsReportingExportActions report={report} disabled={busy} /> : null}
      ...
      <div className={styles.workspace} data-testid="logistics-reporting-workspace">
        ...
        {report ? <>
          <div className={styles.notice}>
            <strong>Nguồn quản trị dùng chung:</strong> Báo cáo quản trị dùng cùng nguồn số liệu này. Đúng hạn chỉ tính phiếu <strong>giao đủ</strong> có giờ dự kiến tại điểm giao; dòng thiếu SLA bị loại khỏi mẫu số và hiển thị riêng, không tự coi là đúng hạn.
          </div>
          ...
          <WorkspaceTabs
            ...
            idPrefix={LOGISTICS_TAB_PREFIX}
            label="Chi tiết hiệu suất giao hàng"
          />
          ...
          <WorkspaceTabPanel tabId="results" activeTab={activeTab} idPrefix={LOGISTICS_TAB_PREFIX}>
          ...
          <WorkspaceTabPanel tabId="trips" activeTab={activeTab} idPrefix={LOGISTICS_TAB_PREFIX}>
            <section className={styles.section} data-testid="logistics-trips-panel">
              <div className={styles.sectionHeader}><div><h2>100 chuyến gần nhất trong bộ lọc</h2><p>Cho phép đối chiếu về đúng màn chuyến giao và phiếu giao hàng đang sử dụng.</p></div><div className={styles.headerActions}><Link className={styles.linkButton} href="/logistics/trips">Chuyến giao</Link><Link className={styles.linkButton} href="/inventory/delivery-orders">Phiếu giao hàng</Link><Link className={styles.linkButton} href="/logistics/trip-reconciliation">Đối soát cuối chuyến</Link></div></div>
              <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Chuyến</th><th>Kho</th><th>Tài xế / xe</th><th>Thời gian</th><th>Trạng thái</th><th className={styles.numeric}>Điểm / phiếu</th><th className={styles.numeric}>Full / partial</th><th className={styles.numeric}>Fail / hẹn</th><th className={styles.numeric}>Đúng hạn</th><th className={styles.numeric}>Chưa kết quả</th></tr></thead><tbody>
                {report.trips.map((row) => <tr key={row.tripId}><td><strong>{row.tripNumber}</strong></td><td>{row.warehouseCode}<br />{row.warehouseName}</td><td>{row.driverCode ?? '—'} · {row.driverName ?? '—'}<br />{row.vehicleCode ?? '—'} · {row.licensePlate ?? '—'}</td><td>KH {timestamp(row.plannedStartAt)}<br />Đi {timestamp(row.dispatchedAt)}<br />Đóng {timestamp(row.closedAt)}</td><td>{tripStatusLabel(row.status)}</td><td className={styles.numeric}>{count(row.stopCount)} / {count(row.deliveryOrderCount)}</td><td className={styles.numeric}>{count(row.deliveredFullCount)} / {count(row.deliveredPartialCount)}</td><td className={styles.numeric}>{count(row.failedCount)} / {count(row.rescheduledCount)}</td><td className={styles.numeric}>{percent(row.onTimeFullRatePercent)}</td><td className={styles.numeric}>{count(row.pendingResultCount)}</td></tr>)}
