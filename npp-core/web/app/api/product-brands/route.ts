...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await listProductBrands<unknown[]>(requestId, request.nextUrl.searchParams);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await createProductBrand(requestId, body, request.headers.get('idempotency-key') ?? undefined);
