...
import { normalizeVariantQuantity, normalizeProductGatewayError, resolveProductRequestId } from '../../../../../../../lib/product-gateway';
...
export async function POST(request: NextRequest, { params }: { params: { id: string; variantId: string } }) {
  ...
  try {
    const data = await normalizeVariantQuantity(params.id, params.variantId, requestId, parsed.body);
