...
import { getVariantUnit, patchVariantUnit, normalizeProductGatewayError, resolveProductRequestId } from '../../../../../../../lib/product-gateway';
...
export async function GET(request: NextRequest, { params }: Params) {
  ...
  try {
    const data = await getVariantUnit<unknown>(params.id, params.variantId, requestId);
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  try {
    const data = await patchVariantUnit(params.id, params.variantId, requestId, parsed.body);
    return NextResponse.json({ data, requestId }, { status: 200, headers: responseHeaders(requestId) });
  } catch (error) { return errorResponse(error, requestId); }
}
