...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try { return backupData(await requestBackupApi({ path: `/api/data-deletions/${params.id}/execute`, method: 'POST', body: parsed.body, idempotencyKey: backupIdempotencyKey(request), requestId: backupRequestId(request) })); }
