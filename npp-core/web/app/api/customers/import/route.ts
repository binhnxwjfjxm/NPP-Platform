...
import { importCustomers } from '../../../../lib/customer-bulk-gateway';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const data = await importCustomers(requestId, body, dryRun ? undefined : request.headers.get('idempotency-key') ?? undefined);
