...
import { bulkUpdateCustomers } from '../../../../lib/customer-bulk-gateway';
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const data = await bulkUpdateCustomers(requestId, body, dryRun ? undefined : request.headers.get('idempotency-key') ?? undefined);
