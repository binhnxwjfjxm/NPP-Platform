...
function errorResponse(error: unknown, requestId: string) {
  const normalized = normalizeEmployeeGatewayError(error);
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getEmployeeOrganization<unknown>(requestId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await saveEmployeeOrganization<unknown>(
      requestId,
      body,
