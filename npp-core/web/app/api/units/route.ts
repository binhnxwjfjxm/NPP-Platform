...
import { createUnit, listUnits, normalizeProductGatewayError, resolveProductRequestId } from '../../../lib/product-gateway';
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await listUnits<unknown>(requestId, request.nextUrl.searchParams);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await createUnit(requestId, parsed.body, request.headers.get('idempotency-key') ?? undefined);
