...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await listProductCategories<unknown[]>(requestId, request.nextUrl.searchParams);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await createProductCategory(requestId, body, request.headers.get('idempotency-key') ?? undefined);
