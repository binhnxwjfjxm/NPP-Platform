...
import { normalizeWorkforceGatewayError, recordOvertimeActual, resolveWorkforceRequestId } from '../../../../../lib/workforce-gateway';
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await recordOvertimeActual(requestId, body, request.headers.get('idempotency-key') ?? undefined);
