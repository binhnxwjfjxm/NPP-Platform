...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await listOvertimeRequests<unknown>(requestId, request.nextUrl.searchParams);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await submitOvertimeRequest(requestId, body, request.headers.get('idempotency-key') ?? undefined);
