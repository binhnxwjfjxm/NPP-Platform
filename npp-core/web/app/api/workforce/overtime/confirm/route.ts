...
import { confirmOvertimeRequest, normalizeWorkforceGatewayError, resolveWorkforceRequestId } from '../../../../../lib/workforce-gateway';
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await confirmOvertimeRequest(requestId, body, request.headers.get('idempotency-key') ?? undefined);
