...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await listLeaveRequests<unknown>(requestId, request.nextUrl.searchParams);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await submitLeaveRequest(requestId, parsed.body, request.headers.get('idempotency-key') ?? undefined);
