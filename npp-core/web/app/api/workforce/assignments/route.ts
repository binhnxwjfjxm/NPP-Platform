...
import {
  assignEmployeeWorkPolicy,
  listEmployeePolicyAssignments,
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await listEmployeePolicyAssignments<unknown[]>(requestId, employeeId);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await assignEmployeeWorkPolicy(requestId, body, request.headers.get('idempotency-key') ?? undefined);
