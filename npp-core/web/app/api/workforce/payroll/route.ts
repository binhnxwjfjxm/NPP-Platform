...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getPayrollFoundation<unknown>(requestId, request.nextUrl.searchParams);
...
export async function POST(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        error: {
          ...
          message: 'Nội dung yêu cầu phải là JSON hợp lệ',
          retryable: false,
        },
        requestId,
      },
  ...
  try {
    const data = await mutatePayrollFoundation(
      requestId,
      body,
