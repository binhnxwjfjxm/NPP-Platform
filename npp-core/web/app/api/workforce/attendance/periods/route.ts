...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await listAttendancePeriods<unknown>(requestId, request.nextUrl.searchParams);
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await mutateAttendancePeriod(requestId, body, request.headers.get('idempotency-key') ?? undefined);
    return NextResponse.json({ data, requestId }, { status: 200, headers: headers(requestId) });
  } catch (error) { return errorResponse(error, requestId); }
}
