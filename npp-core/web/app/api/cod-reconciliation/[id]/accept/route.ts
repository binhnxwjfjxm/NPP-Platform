...
import { acceptCodHandover } from '../../../../../lib/cod-reconciliation-gateway';
...
export async function POST(request: NextRequest, context: { params: { id: string } }) {
  ...
  try {
    const data = await acceptCodHandover<unknown>(context.params.id, await codBody(request), requestId, request.headers.get('idempotency-key') || '');
