...
import { importProductUnits, normalizeProductGatewayError, resolveProductRequestId } from '../../../../lib/product-gateway';
...
export async function POST(request: NextRequest) {
  ...
  try {
    const data = await importProductUnits(requestId, parsed.body, request.headers.get('idempotency-key') ?? undefined);
