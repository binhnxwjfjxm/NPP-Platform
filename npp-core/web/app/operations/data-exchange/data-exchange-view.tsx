  ...
  return <AppShell kicker="Dữ liệu vận hành" title="Nhập/xuất dữ liệu và báo giá" subtitle="Nhập, kiểm tra và xuất dữ liệu theo từng nghiệp vụ." actions={actions}>
    <div className={styles.page} data-testid="phase-10-4-data-exchange">
      ...
      {tab === 'products' ? <section className={styles.panel}><div className={styles.panelTitle}><div><h2>Sản phẩm và SKU</h2><p>Nhập hoặc xuất nhiều sản phẩm và SKU cùng lúc. Sau khi chọn tệp, xem trước và chọn đơn vị, lô, hạn sử dụng và vị trí kho trước khi ghi dữ liệu.</p></div><div className={styles.buttonRow}>
        {fileInput('products', '.xlsx,.csv', 'products')}<button type="button" className={styles.primaryButton} onClick={() => fileRefs.current.products?.click()} disabled={busy}>Chọn tệp để nhập</button>
        <button type="button" className={styles.secondaryButton} onClick={() => void productTemplate('xlsx')} disabled={busy}>Tải mẫu Excel</button><button type="button" className={styles.secondaryButton} onClick={() => void productTemplate('csv')} disabled={busy}>Tải mẫu CSV</button>
      ...
      </div></div>{columnChooser(PRODUCT_COLUMNS, productColumns, setProductColumns)}<div className={styles.guardrail}><strong>Cách khai báo SKU</strong><span>Mỗi SKU chọn đơn vị tính và hệ số quy đổi. SKU dùng làm đơn vị tồn chuẩn có hệ số = 1; tại SKU này chọn thêm Có/Không quản lý theo lô, cách quản lý hạn sử dụng và có bắt buộc vị trí kho hay không.</span></div><p className={styles.note}>Khi cập nhật sản phẩm đã có, tệp cần chứa đầy đủ các SKU đang sử dụng của sản phẩm đó để tránh bỏ sót quy cách.</p>{previewTable()}</section> : null}

      {tab === 'pricing' ? <section className={styles.panel}><div className={styles.panelTitle}><div><h2>Cập nhật giá bán theo SKU</h2><p>Chọn một bảng giá hoặc chương trình đang hoạt động. Tệp chỉ cần Mã hàng (SKU) và Giá bán (VND). SKU đã có giá sẽ được cập nhật; SKU chưa có giá sẽ được thêm mới.</p></div><div className={styles.buttonRow}>{fileInput('pricing', '.xlsx,.csv', 'pricing')}<button type="button" className={styles.primaryButton} onClick={() => fileRefs.current.pricing?.click()} disabled={busy || !pricingPriceListId}>Chọn tệp giá</button><button type="button" className={styles.secondaryButton} onClick={() => void pricingTemplate('xlsx')} disabled={busy}>Tải mẫu Excel</button><button type="button" className={styles.secondaryButton} onClick={() => void pricingTemplate('csv')} disabled={busy}>Tải mẫu CSV</button><button type="button" className={styles.secondaryButton} onClick={() => void pricingExport('xlsx')} disabled={busy || !pricingPriceListId}>Xuất giá hiện tại Excel</button><button type="button" className={styles.secondaryButton} onClick={() => void pricingExport('csv')} disabled={busy || !pricingPriceListId}>Xuất giá hiện tại CSV</button></div></div><label className={styles.field}>Bảng giá hoặc chương trình cần cập nhật<select value={pricingPriceListId} onChange={(event) => { setPricingPriceListId(event.target.value); setPendingImport(null); setError(''); setMessage(''); }}><option value="">Chọn bảng giá hoặc chương trình</option>{priceLists.filter((list) => list.is_active).map((list) => <option key={list.id} value={list.id}>{list.code} · {list.name}</option>)}</select></label><div className={styles.guardrail}><strong>Mẫu nhập luôn có tiêu đề SKU và Giá bán (VND).</strong><span>Hỗ trợ Excel .xlsx và CSV .csv. Nếu dùng tệp Excel .xls cũ, hãy lưu lại thành .xlsx trước khi nhập.</span></div>{previewTable()}</section> : null}

      {tab === 'stocktake' ? <section className={styles.panel}><div className={styles.panelTitle}><div><h2>Nhập số kiểm kê thực tế</h2><p>Chọn kho, tải tệp kiểm kê, điền số đếm thực tế rồi nhập lại. Hệ thống chỉ tạo phiếu kiểm kê nháp; tồn kho chưa thay đổi cho đến khi phiếu được duyệt và ghi sổ.</p></div><div className={styles.buttonRow}>{fileInput('stocktake', '.xlsx,.csv', 'stocktake')}<button type="button" className={styles.primaryButton} onClick={() => fileRefs.current.stocktake?.click()} disabled={busy}>Chọn tệp số đếm</button><button type="button" className={styles.secondaryButton} onClick={() => void stocktakeExport('xlsx')} disabled={busy}>Tải mẫu Excel</button><button type="button" className={styles.secondaryButton} onClick={() => void stocktakeExport('csv')} disabled={busy}>Tải mẫu CSV</button></div></div><label className={styles.field}>Kho kiểm kê<select value={stocktakeWarehouse} onChange={(event) => setStocktakeWarehouse(event.target.value)}><option value="">Chọn kho</option>{warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.code} · {warehouse.name}</option>)}</select></label><div className={styles.guardrail}><strong>Kiểm kê không thay đổi tồn ngay.</strong><span>Số đếm thực tế → tạo phiếu kiểm kê nháp → gửi duyệt → duyệt → ghi sổ chênh lệch. Chưa gửi duyệt, chưa ghi sổ tồn.</span><a href="/inventory/stocktakes">Mở màn Kiểm kê kho</a></div>{previewTable()}</section> : null}

      {tab === 'quotation' ? <section className={styles.panel}><div className={styles.panelTitle}><div><h2>Báo giá</h2><p>Chọn phạm vi hàng hóa và khách hàng; hệ thống tự tìm mức giá đang áp dụng cho từng SKU.</p></div><div className={styles.buttonRow}><button type="button" className={styles.primaryButton} onClick={() => void buildQuotation()} disabled={busy}>Tính báo giá</button><button type="button" className={styles.secondaryButton} onClick={() => void quotationExport('xlsx')} disabled={busy || !quotationRows.length}>Xuất Excel</button><button type="button" className={styles.secondaryButton} onClick={() => void quotationExport('csv')} disabled={busy || !quotationRows.length}>Xuất CSV</button></div></div><div className={styles.formGrid}>
        <label className={styles.field}>Phạm vi<select value={quotationScope} onChange={(event) => setQuotationScope(event.target.value as 'all' | 'category' | 'sku')}><option value="all">Tất cả SKU đang bán</option><option value="category">Theo ngành hoặc nhóm</option><option value="sku">Danh sách SKU</option></select></label>
        {quotationScope === 'category' ? <label className={styles.field}>Ngành hoặc nhóm<select value={quotationCategory} onChange={(event) => setQuotationCategory(event.target.value)}><option value="">Chọn nhóm</option>{categories.filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</select></label> : null}
        {quotationScope === 'sku' ? <label className={`${styles.field} ${styles.span2}`}>SKU<input value={quotationSkus} onChange={(event) => setQuotationSkus(event.target.value)} placeholder="Ví dụ: SP001-LE, SP002-THUNG" /></label> : null}
        <label className={styles.field}>Kênh bán<select value={quotationContext.channelId} onChange={(event) => setQuotationContext({ ...quotationContext, channelId: event.target.value })}><option value="">Không chọn</option>{channels.filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</select></label>
