...
import {
  type Tab, type Balance, type MovementView, type PriceList,
  PRODUCT_COLUMNS,
...
export function DataExchangeView({ ctx }: { ctx: DataExchangeViewContext }) {
  const {
    ...
    setStocktakeWarehouse, warehouses, loadMovements, loadMoreMovements, movementHasMore, exportMovements, selectedBalanceKey, selectMovementBalance, balances, selectedBalance, movementRows,
  ...
  return <AppShell kicker="Dữ liệu vận hành" title="Nhập/xuất dữ liệu" subtitle="Nhập, xuất dữ liệu nghiệp vụ và tải biểu mẫu trống phục vụ công việc văn phòng." actions={actions}>
    <div className={styles.page} data-testid="phase-10-4-data-exchange">
      <nav className={styles.tabs} aria-label="Nhóm dữ liệu">{([['products', 'Sản phẩm và SKU'], ['pricing', 'Giá bán'], ['stocktake', 'Kiểm kê'], ['movements', 'Biến động kho'], ['office-forms', 'Biểu mẫu văn phòng']] as Array<[Tab, string]>).map(([key, label]) => <button key={key} type="button" className={tab === key ? styles.activeTab : ''} onClick={() => { setTab(key); setError(''); setMessage(''); setPendingImport(null); }}>{label}</button>)}</nav>
