...
type StocktakeLine = { id: string; locationId: string | null; locationCode: string | null; baseVariantId: string; baseSku: string; lotId: string | null; lotCode: string | null };
type Stocktake = { id: string; stocktakeNumber: string; revision: string; status: string; lines?: StocktakeLine[] };
type PricingResolution = {
  variant: { id: string; sku: string; name: string; product_code?: string; product_name?: string };
  currencyCode: string; baseUnitPriceMinor: string; finalUnitPriceMinor: string; lineTotalMinor: string;
};
type Movement = {
...
function trimDecimal(value: string) {
  ...
  const next = normalized.replace(/0+$/, '').replace(/\.$/, '');
...
function scaled12(value: string) {
  ...
  const absolute = BigInt(match[2]) * 1_000_000_000_000n + BigInt((match[3] ?? '').padEnd(12, '0'));
...
export default function DataExchangeWorkspace() {
  ...
  const [pricingColumns, setPricingColumns] = useState<Set<string>>(new Set(PRICING_COLUMNS));
  const [stocktakeWarehouse, setStocktakeWarehouse] = useState('');
  const [quotationScope, setQuotationScope] = useState<'all' | 'category' | 'sku'>('all');
  const [quotationCategory, setQuotationCategory] = useState('');
  const [quotationSkus, setQuotationSkus] = useState('');
  const [quotationContext, setQuotationContext] = useState({ channelId: '', customerGroupId: '', customerId: '', quantity: '1' });
  const [quotationRows, setQuotationRows] = useState<QuotationRow[]>([]);
  const [selectedBalanceKey, setSelectedBalanceKey] = useState('');
  const [movementRows, setMovementRows] = useState<MovementView[]>([]);
  ...
  function toggleColumn(setter: (value: Set<string>) => void, current: Set<string>, column: string) {
    const next = new Set(current); if (next.has(column)) next.delete(column); else next.add(column); setter(next);
  }
  ...
  async function productExport(format: 'xlsx' | 'csv') {
    ...
    try {
      ...
      const selected = PRODUCT_COLUMNS.filter((column) => productColumns.has(column) && result.columns.includes(column));
      if (!selected.length) throw new Error('Chọn ít nhất một cột để xuất.');
      const rows = result.rows.map((row) => selected.map((column) => String(row[column] ?? '')));
      await exportTable('san-pham-sku.xlsx', 'Sản phẩm SKU', selected, rows, format);
  ...
  async function productImport(file: File) {
    ...
    try {
      ...
      requireColumns(rows, ['productCode', 'productName', 'productIsCatalogVisible', 'productIsOrderable', 'productIsActive', 'sku', 'skuName', 'variantKind', 'isInventoryBase', 'isSellable', 'isCatalogVisible', 'isActive']);
      const result = await requestJson<{ jobId?: string; import?: { imported?: number; created?: number; updated?: number } }>('/api/file-operations/products/import', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotency('p10_product_import') }, body: JSON.stringify({ format: file.name.toLowerCase().endsWith('.xlsx') ? 'xlsx' : 'csv', rows }) });
      await refreshReferenceData();
      const info = result.import ?? {};
      setMessage(`Đã import sản phẩm/SKU${info.imported == null ? '' : `: ${info.imported} sản phẩm`}${result.jobId ? ` · job ${result.jobId}` : ''}.`);
    } catch (cause) { fail(cause); } finally { setBusy(false); }
  }
  ...
  async function loadPricingItems() {
    ...
    await Promise.all(priceLists.map(async (list) => {
      const items = await requestJson<PriceItem[]>(`/api/price-lists/${list.id}/items?limit=2000`);
      for (const item of items) rows.push({ list, item });
    }));
    return rows;
  }
  ...
  async function pricingExport(format: 'xlsx' | 'csv') {
    ...
    try {
      ...
      const selected = PRICING_COLUMNS.filter((column) => pricingColumns.has(column) && result.columns.includes(column));
      if (!selected.length) throw new Error('Chọn ít nhất một cột để xuất.');
      const rows = result.rows.map((row) => selected.map((column) => String(row[column] ?? '')));
      await exportTable('bang-gia-sku.xlsx', 'Giá bán SKU', selected, rows, format);
      setMessage(`Đã xuất ${rows.length} dòng giá${result.jobId ? ` · job ${result.jobId}` : ''}.`);
    } catch (cause) { fail(cause); } finally { setBusy(false); }
  }
  ...
  async function pricingImport(file: File) {
    ...
    try {
      const rows = await readTable(file);
      requireColumns(rows, ['priceListCode', 'sku', 'adjustmentType', 'sourceKey', 'isActive']);
      if (rows.length > 2000) throw new Error('Import giá tối đa 2.000 dòng.');
      const blankSource = rows.filter((row) => !String(row.sourceKey ?? '').trim());
      const officialRows = rows.filter((row) => String(row.sourceKey ?? '').trim());
      let officialJob = '';
      if (officialRows.length) {
        const result = await requestJson<{ jobId?: string }>('/api/file-operations/pricing/import', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': idempotency('p10_pricing_import') }, body: JSON.stringify({ format: file.name.toLowerCase().endsWith('.xlsx') ? 'xlsx' : 'csv', rows: officialRows }) });
        officialJob = result.jobId ?? '';
      }
      if (blankSource.length) {
        const listByCode = new Map<string, PriceList>(priceLists.map((item) => [item.code.toUpperCase(), item] as const));
        const existingRows = await loadPricingItems();
  ...
  async function stocktakeExport(format: 'xlsx' | 'csv') {
    ...
    try {
      ...
      const selected = STOCKTAKE_COLUMNS.filter((column) => result.columns.includes(column));
      const rows = result.rows.map((row) => selected.map((column) => String(row[column] ?? '')));
      await exportTable(`kiem-ke-${warehouse.code}.xlsx`, 'Kiểm kê thực tế', selected, rows, format);
      setMessage(`Đã xuất ${rows.length} phạm vi kiểm kê; file không chứa số tồn hệ thống${result.jobId ? ` · job ${result.jobId}` : ''}.`);
    } catch (cause) { fail(cause); } finally { setBusy(false); }
  }
  ...
  async function stocktakeImport(file: File) {
    begin();
    try {
      const rows = await readTable(file); requireColumns(rows, STOCKTAKE_COLUMNS);
      if (rows.length > 500) throw new Error('Mỗi đợt kiểm kê tối đa 500 phạm vi.');
  ...
  async function buildQuotation() {
    begin();
    try {
      const quantity = exactQuantity(quotationContext.quantity, 'Số lượng báo giá', 6); const manualSkus = new Set<string>(quotationSkus.split(/[\s,;]+/).map((value) => value.trim().toUpperCase()).filter((value) => value.length > 0));
      let selectedProducts = products.filter((product) => product.is_active);
      if (quotationScope === 'category') { if (!quotationCategory) throw new Error('Chọn ngành/nhóm sản phẩm.'); selectedProducts = selectedProducts.filter((product) => product.category_id === quotationCategory); }
      const skus: string[] = [];
      await Promise.all(selectedProducts.map(async (product) => {
        const variants = await requestJson<Variant[]>(`/api/products/${product.id}/variants`);
        variants.filter((variant) => variant.is_active && variant.is_sellable && (quotationScope !== 'sku' || manualSkus.has(variant.sku.toUpperCase()))).forEach((variant) => skus.push(variant.sku));
      }));
      const unique = [...new Set(skus)].sort();
      if (quotationScope === 'sku') { const found = new Set(unique.map((sku) => sku.toUpperCase())); const missing = [...manualSkus].filter((sku) => !found.has(sku)); if (missing.length) throw new Error(`Không tìm thấy SKU bán được: ${missing.join(', ')}.`); }
  ...
  async function loadMovements() {
    ...
    try {
      ...
      const params = new URLSearchParams({ warehouseId: selectedBalance.warehouse_id, baseVariantId: selectedBalance.base_variant_id, limit: '500' });
      ...
      const rows = await requestJson<Movement[]>(`/api/inventory/balances/drill-down?${params}`);
      let running = scaled12(selectedBalance.on_hand_quantity);
      const views = rows.map((row) => { const stockAfter = formatScaled12(running); running -= scaled12(row.base_quantity_delta); return { ...row, stockAfter }; });
      setMovementRows(views); setMessage(`Đã tải ${views.length} biến động của ${selectedBalance.base_sku}.`);
    } catch (cause) { fail(cause); } finally { setBusy(false); }
  }
  ...
  return <AppShell kicker="Phase 10.4 · Dữ liệu & báo giá" title="Import / Export & Báo giá" subtitle="Tệp chỉ đi qua API nghiệp vụ canonical; tồn kho thực tế đi qua kiểm kê và Inventory Ledger, không sửa balance trực tiếp." actions={actions}>
    <div className={styles.page} data-testid="phase-10-4-data-exchange">
      <nav className={styles.tabs} aria-label="Nhóm dữ liệu Phase 10.4">
        {([['products', 'Sản phẩm / SKU'], ['pricing', 'Giá bán'], ['stocktake', 'Kiểm kê'], ['quotation', 'Báo giá'], ['movements', 'Biến động kho']] as Array<[Tab, string]>).map(([key, label]) => <button key={key} type="button" className={tab === key ? styles.activeTab : ''} onClick={() => { setTab(key); setError(''); setMessage(''); }}>{label}</button>)}
      </nav>
      ...
      {tab === 'products' ? <section className={styles.panel}><div className={styles.panelTitle}><div><h2>Sản phẩm / SKU</h2><p>SKU là khóa cập nhật người dùng. File đi qua official file-operation API rồi tái sử dụng product import hiện hữu.</p></div><div className={styles.buttonRow}>
      ...
      </div></div>{columnChooser(PRODUCT_COLUMNS, productColumns, setProductColumns)}<p className={styles.note}>Cập nhật sản phẩm hiện hữu phải chứa đầy đủ SKU đang hoạt động của sản phẩm; Core sẽ từ chối snapshot thiếu thay vì tự xóa/ngưng SKU.</p></section> : null}

      {tab === 'pricing' ? <section className={styles.panel}><div className={styles.panelTitle}><div><h2>Giá bán theo SKU</h2><p>Định danh file: priceListCode + SKU + sourceKey. Dòng legacy sourceKey trống được nhận diện và PATCH bằng optimistic revision; dòng canonical đi qua pricing import.</p></div><div className={styles.buttonRow}>
        {fileInput('pricing', '.xlsx,.csv', pricingImport)}<button type="button" className={styles.primaryButton} onClick={() => fileRefs.current.pricing?.click()} disabled={busy}>Nhập file</button>
        <button type="button" className={styles.secondaryButton} onClick={() => void pricingExport('xlsx')} disabled={busy}>Xuất XLSX</button><button type="button" className={styles.secondaryButton} onClick={() => void pricingExport('csv')} disabled={busy}>Xuất CSV</button>
      </div></div>{columnChooser(PRICING_COLUMNS, pricingColumns, setPricingColumns)}</section> : null}

      {tab === 'stocktake' ? <section className={styles.panel}><div className={styles.panelTitle}><div><h2>Nhập số kiểm kê thực tế</h2><p>File tạo đợt kiểm kê và ghi số đếm. Hệ thống dừng ở trạng thái counted; chưa gửi duyệt và chưa ghi sổ chênh lệch.</p></div><div className={styles.buttonRow}>
        {fileInput('stocktake', '.xlsx,.csv', stocktakeImport)}<button type="button" className={styles.primaryButton} onClick={() => fileRefs.current.stocktake?.click()} disabled={busy}>Nhập số đếm</button>
        <button type="button" className={styles.secondaryButton} onClick={() => void stocktakeExport('xlsx')} disabled={busy}>Mẫu XLSX</button><button type="button" className={styles.secondaryButton} onClick={() => void stocktakeExport('csv')} disabled={busy}>Mẫu CSV</button>
      </div></div><label className={styles.field}>Kho kiểm kê<select value={stocktakeWarehouse} onChange={(event) => setStocktakeWarehouse(event.target.value)}><option value="">Chọn kho</option>{warehouses.map((warehouse) => <option key={warehouse.id} value={warehouse.id}>{warehouse.code} · {warehouse.name}</option>)}</select></label>
        <div className={styles.guardrail}><strong>Không cập nhật tồn trực tiếp.</strong><span>actual_quantity → Stocktake draft/count → gửi duyệt → duyệt → Ghi sổ chênh lệch → STOCKTAKE_ADJUSTMENT ledger. Các bước sau import vẫn dùng màn Kiểm kê hiện hữu.</span><a href="/inventory/stocktakes">Mở Kiểm kê kho</a></div>
      </section> : null}

      {tab === 'quotation' ? <section className={styles.panel}><div className={styles.panelTitle}><div><h2>Báo giá</h2><p>Báo giá đi qua official file-operation API; backend gọi pricing resolver hiện hữu cho từng SKU và ghi job history.</p></div><div className={styles.buttonRow}><button type="button" className={styles.primaryButton} onClick={() => void buildQuotation()} disabled={busy}>Tính báo giá</button><button type="button" className={styles.secondaryButton} onClick={() => void quotationExport('xlsx')} disabled={busy || !quotationRows.length}>Xuất XLSX</button><button type="button" className={styles.secondaryButton} onClick={() => void quotationExport('csv')} disabled={busy || !quotationRows.length}>Xuất CSV</button></div></div>
        <div className={styles.formGrid}><label className={styles.field}>Phạm vi<select value={quotationScope} onChange={(event) => setQuotationScope(event.target.value as 'all' | 'category' | 'sku')}><option value="all">Tất cả SKU bán được</option><option value="category">Theo ngành / nhóm</option><option value="sku">Danh sách SKU</option></select></label>
      ...
      {tab === 'movements' ? <section className={styles.panel}><div className={styles.panelTitle}><div><h2>Tra cứu biến động theo SKU</h2><p>Balance trả lời còn bao nhiêu; ledger drill-down trả lời từng lần +/− và chứng từ nguồn.</p></div><button type="button" className={styles.primaryButton} onClick={() => void loadMovements()} disabled={busy}>Truy vấn biến động</button></div>
        ...
        {movementRows.length ? <div className={styles.tableWrap}><table><thead><tr><th>Thời gian</th><th>Chứng từ</th><th>Loại</th><th>Biến động</th><th>Tồn sau</th><th>Người ghi sổ</th></tr></thead><tbody>{movementRows.map((row) => <tr key={`${row.movement_id}:${row.source_line_reference ?? row.base_quantity_delta}`}><td>{new Intl.DateTimeFormat('vi-VN', { dateStyle: 'short', timeStyle: 'short', timeZone: 'Asia/Ho_Chi_Minh' }).format(new Date(row.posted_at))}</td><td>{row.source_document_number || row.document_number || row.source_document_type || '—'}</td><td>{row.movement_type}</td><td className={scaled12(row.base_quantity_delta) >= 0n ? styles.positive : styles.negative}>{scaled12(row.base_quantity_delta) >= 0n ? '+' : ''}{trimDecimal(row.base_quantity_delta)}</td><td>{trimDecimal(row.stockAfter)}</td><td>{row.posted_by}</td></tr>)}</tbody></table></div> : null}
      </section> : null}
    </div>
  </AppShell>;
}
