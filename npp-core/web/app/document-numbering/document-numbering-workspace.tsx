...
function seriesToForm(series: DocumentNumberSeries): DocumentNumberSeriesForm {
  return {
    code: series.code, documentType: series.document_type, name: series.name,
    prefix: series.prefix, numberTemplate: series.number_template,
    resetPolicy: series.reset_policy, sequenceWidth: String(series.sequence_width),
    startCounter: String(series.start_counter), timezoneName: series.timezone_name,
    description: series.description ?? '', isActive: series.is_active,
  };
}
...
export default function DocumentNumberingWorkspace() {
  ...
  async function selectSeries(item: DocumentNumberSeries) {
    setBusy(true); setError(null); setNotice(null); setLastAllocation(null);
    try {
      setSelected(item);
  ...
  function openCreate() {
    setEditing(null); setForm(EMPTY_FORM); setShowForm(true); setError(null); setNotice(null);
  }

  function openEdit(item: DocumentNumberSeries) {
    setEditing(item); setForm(seriesToForm(item)); setShowForm(true); setError(null); setNotice(null);
  }
  ...
  async function saveSeries() {
    ...
    try {
      const body = {
        ...form, sequenceWidth: Number(form.sequenceWidth), startCounter: form.startCounter,
      ...
      setShowForm(false); setEditing(saved);
  ...
  async function allocateTestNumber() {
    ...
    try {
      const saved = await requestJson<DocumentNumberAllocation>(`/api/document-number-series/${selected.id}/allocate`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': allocationKey },
      ...
      setNotice(saved.replayed ? 'Đã trả lại số cũ theo khóa idempotency; không cấp thêm số' : 'Đã cấp một số kiểm thử và ghi lịch sử bất biến');
    } catch (value) {
    ...
    <AppShell title="Số chứng từ" subtitle="Quản lý series và cấp số an toàn; màn này không tạo chứng từ nghiệp vụ" kicker="NPP Document Numbering">
      <main className={styles.workspace} data-testid="document-numbering-page">
        <section className={styles.hero}>
          <div><span className={styles.eyebrow}>Phase 3.3F</span><h2>Bộ máy cấp số dùng chung</h2><p>Cấu hình mẫu số theo từng loại chứng từ. Series đã phát sinh số sẽ khóa định dạng để bảo vệ lịch sử.</p></div>
        ...
        <section className={styles.panel}>
          <div className={styles.filters}>
            ...
            <select value={activeFilter} onChange={(event) => setActiveFilter(event.target.value as typeof activeFilter)}><option value="all">Tất cả trạng thái</option><option value="active">Đang hoạt động</option><option value="inactive">Đã ngừng</option></select>
            <button type="button" className={styles.secondaryButton} disabled={busy} onClick={() => void loadSeries(selected?.id)}>Làm mới</button>
          </div>

          {showForm ? (
            <div className={styles.formPanel} data-testid="number-series-form">
              <div className={styles.formHeader}><div><h3>{editing ? `Sửa ${editing.code}` : 'Tạo series số chứng từ'}</h3>{editing?.format_locked ? <p>Định dạng đã khóa vì series có lịch sử cấp số.</p> : null}</div><button type="button" onClick={() => setShowForm(false)}>Đóng</button></div>
              ...
              <button type="button" className={styles.primaryButton} disabled={busy} onClick={() => void saveSeries()} data-testid="save-number-series-button">Lưu series</button>
            </div>
          ) : null}

          <div className={styles.tableWrapper}><table className={styles.table}><thead><tr><th>Mã</th><th>Loại chứng từ</th><th>Mẫu</th><th>Reset</th><th>Đã cấp</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>
            {visibleSeries.map((item) => <tr key={item.id} data-testid={`number-series-row-${item.code}`} className={selected?.id === item.id ? styles.selectedRow : undefined}><td><strong>{item.code}</strong><small>{item.name}</small></td><td>{item.document_type}</td><td><code>{item.number_template}</code>{item.format_locked ? <span className={styles.locked}>Đã khóa</span> : null}</td><td>{item.reset_policy}</td><td>{item.allocation_count}</td><td>{item.is_active ? 'Hoạt động' : 'Ngừng'}</td><td className={styles.actions}><button type="button" onClick={() => void selectSeries(item)} data-testid={`select-number-series-${item.code}`}>Chi tiết</button><button type="button" onClick={() => openEdit(item)}>Sửa</button><button type="button" disabled={busy} onClick={() => void toggleActive(item)}>{item.is_active ? 'Vô hiệu' : 'Kích hoạt'}</button></td></tr>)}
        ...
        {selected ? (
          <section className={styles.detailGrid} data-testid="number-series-detail">
            <div className={styles.panel}>
              <div className={styles.sectionHeader}><div><span className={styles.eyebrow}>Cấp số kiểm thử</span><h3>{selected.code} — {selected.name}</h3></div><span className={selected.is_active ? styles.activeBadge : styles.inactiveBadge}>{selected.is_active ? 'Hoạt động' : 'Ngừng'}</span></div>
              ...
              <div className={styles.formGrid}><label>Ngày chứng từ<input type="date" value={documentDate} onChange={(event) => setDocumentDate(event.target.value)} data-testid="allocation-date-input" /></label><label>Khóa idempotency<input value={allocationKey} onChange={(event) => setAllocationKey(event.target.value)} data-testid="allocation-key-input" /></label></div>
              <div className={styles.actionsBar}><button type="button" className={styles.primaryButton} disabled={busy || !selected.is_active || !documentDate || !allocationKey.trim()} onClick={() => void allocateTestNumber()} data-testid="allocate-test-number-button">Cấp số kiểm thử</button><button type="button" className={styles.secondaryButton} onClick={() => setAllocationKey(idempotencyKey('test-number'))}>Tạo khóa mới</button></div>
              {lastAllocation ? <div className={styles.result} data-testid="allocation-result"><span>{lastAllocation.replayed ? 'Replay' : 'Số mới'}</span><strong>{lastAllocation.document_number}</strong><small>Kỳ {lastAllocation.period_key} · bộ đếm {lastAllocation.counter_value}</small></div> : null}
            </div>

            <div className={styles.panel}><div className={styles.sectionHeader}><div><span className={styles.eyebrow}>Bộ đếm</span><h3>Trạng thái theo kỳ</h3></div></div><div className={styles.counterList}>{history.counters.map((counter) => <div key={counter.period_key} className={styles.counterCard}><strong>{counter.period_key}</strong><span>Số kế tiếp</span><b>{counter.next_counter}</b></div>)}{history.counters.length === 0 ? <p>Chưa phát sinh bộ đếm.</p> : null}</div></div>

            <div className={`${styles.panel} ${styles.fullWidth}`}>
              <div className={styles.sectionHeader}><div><span className={styles.eyebrow}>Lịch sử bất biến</span><h3>Các số đã cấp</h3></div><button type="button" className={styles.secondaryButton} disabled={busy} onClick={() => void loadHistory(selected.id)}>Làm mới lịch sử</button></div>
              <div className={styles.tableWrapper}><table className={styles.table}><thead><tr><th>Số chứng từ</th><th>Ngày chứng từ</th><th>Kỳ</th><th>Bộ đếm</th><th>Cấp lúc</th></tr></thead><tbody>{history.allocations.map((item) => <tr key={item.id} data-testid={`allocation-row-${item.document_number}`}><td><strong>{item.document_number}</strong></td><td>{item.document_date}</td><td>{item.period_key}</td><td>{item.counter_value}</td><td>{new Intl.DateTimeFormat('vi-VN', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'Asia/Ho_Chi_Minh' }).format(new Date(item.allocated_at))}</td></tr>)}{history.allocations.length === 0 ? <tr><td colSpan={5} className={styles.empty}>Chưa có số nào được cấp</td></tr> : null}</tbody></table></div>
            </div>
          </section>
        ) : null}
      </main>
