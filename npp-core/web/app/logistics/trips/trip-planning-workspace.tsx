...
export default function TripPlanningWorkspace() {
  ...
  const idempotencyKey = useCallback((scope: string) => {
    ...
    const next = `web-logistics-${scope}-${crypto.randomUUID()}`.replace(/[^A-Za-z0-9._:-]/g, '_').slice(0, 128);
  ...
  async function tripAction(action: 'assign' | 'unassign' | 'reorder' | 'plan' | 'reopen' | 'lock', body: unknown, discriminator: string) {
    ...
    await runOperation(scope, async () => {
      ...
      setMessage({ assign: 'Đã gán phiếu giao vào chuyến.', unassign: 'Đã bỏ phiếu giao khỏi chuyến.', reorder: 'Đã cập nhật thứ tự điểm giao.', plan: 'Chuyến đã chuyển sang trạng thái lập kế hoạch.', reopen: 'Chuyến đã mở lại để chỉnh sửa.', lock: 'Kế hoạch chuyến đã khóa.' }[action]);
    });
  }
  ...
  return (
    <AppShell kicker="Giao nhận" title="Điều phối giao hàng" subtitle="Lập chuyến, xếp điểm giao, gán xe và tài xế; chưa xuất kho hay ghi kết quả giao.">
        ...
        <div className={styles.columns}>
          <section className={styles.section} aria-labelledby="trip-list-heading">
            ...
            <div className={styles.list} data-testid="trip-list">
              {trips.map((trip) => <button type="button" key={trip.id} className={`${styles.listItem} ${selectedTrip?.id === trip.id ? styles.selected : ''}`} onClick={() => runOperation(`load-${trip.id}`, () => loadTrip(trip.id))}><strong>{trip.number}</strong><span>{trip.warehouseCode || 'Kho'} · {statusLabel(trip.status)}</span><small>{trip.stopCount || 0} điểm · {trip.assignmentCount || 0} phiếu</small></button>)}
            ...
            {selectedTrip ? <>
              ...
              <div className={styles.stopList} data-testid="trip-stop-list">
                {(selectedTrip.stops || []).map((stop, index) => <article className={styles.stopCard} key={stop.id}><header><div><strong>Điểm {stop.sequence}</strong><p>{addressLabel(stop.address)}</p></div>{editable ? <div className={styles.moveButtons}><button type="button" onClick={() => moveStop(stop.id, -1)} disabled={busy !== null || index === 0} aria-label="Đưa điểm giao lên">↑</button><button type="button" onClick={() => moveStop(stop.id, 1)} disabled={busy !== null || index === (selectedTrip.stops?.length || 0) - 1} aria-label="Đưa điểm giao xuống">↓</button></div> : null}</header>
                  {stop.assignments.map((assignment) => <div className={styles.assignment} key={assignment.assignmentId}><span><strong>{assignment.deliveryOrderNumber || assignment.deliveryOrderId.slice(0, 8)}</strong><small>{assignment.customerCode} · {assignment.customerName}</small></span>{editable ? <button type="button" className={styles.textButton} onClick={() => tripAction('unassign', { deliveryOrderId: assignment.deliveryOrderId, reason: 'Điều chỉnh kế hoạch chuyến' }, assignment.deliveryOrderId)} disabled={busy !== null}>Bỏ khỏi chuyến</button> : null}</div>)}
                </article>)}
                {!selectedTrip.stops?.length ? <p className={styles.empty}>Chưa có điểm giao.</p> : null}
              </div>
              <div className={styles.readyQueue}><h3>Phiếu sẵn sàng cùng kho</h3>
                {selectedEligible.map((order) => <div className={styles.readyItem} key={order.id}><span><strong>{order.number || order.id.slice(0, 8)}</strong><small>{order.customerCode} · {order.customerName}</small><small>{formatQuantity(order.totalBaseQuantity)} · {order.lineCount} dòng</small></span>{editable ? <button type="button" className={styles.primaryButton} onClick={() => tripAction('assign', { deliveryOrderId: order.id }, order.id)} disabled={busy !== null} data-testid={`assign-${order.id}`}>Gán vào chuyến</button> : null}</div>)}
                {!selectedEligible.length ? <p className={styles.empty}>Không còn phiếu giao đủ điều kiện trong kho này.</p> : null}
              </div>
              {selectedTrip.status === 'locked' ? <p className={styles.lockNotice} data-testid="locked-read-only">Kế hoạch đã khóa. Xe, tài xế, điểm dừng và phiếu giao chỉ được đọc trong Phase 6E.1.</p> : null}
