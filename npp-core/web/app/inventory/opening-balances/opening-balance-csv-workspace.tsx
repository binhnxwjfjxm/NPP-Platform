...
export default function OpeningBalanceCsvWorkspace({ initialImports, initialError = null }: WorkspaceProps) {
  ...
  useEffect(() => {
    ...
    requestJson<LocationEnvelope>(`/api/inventory/opening-balances/operator/locations?warehouseId=${encodeURIComponent(selectedWarehouseId)}`)
      ...
      .catch((error) => {
        if (!active) return;
        setMessage({ kind: 'error', text: error instanceof Error ? error.message : 'Không tải được vị trí của kho.' });
      })
      .finally(() => { if (active) setBusy(null); });
    return () => { active = false; };
  }, [selectedWarehouseId]);
  ...
  async function chooseFile(file: File) {
    ...
    setRows(parsed);
    setMessage({ kind: 'success', text: `Đã đọc ${parsed.length} dòng. Kiểm tra bảng xem trước rồi bấm “Kiểm tra tệp”.` });
  }
  ...
  async function validate() {
    ...
    try {
      ...
      setMessage(result.rowErrors.length
        ? { kind: 'error', text: 'Có dòng chưa hợp lệ. Xem lỗi bên dưới; chưa có dữ liệu nào được ghi nhận.' }
        : { kind: 'success', text: 'Dữ liệu đã resolve đúng kho, vị trí và SKU. Có thể xác nhận nhập tồn.' });
    } catch (error) {
      if (revision !== draftRevision.current) return;
  ...
  return <AppShell title="Thiết lập tồn đầu kỳ" subtitle="Chọn kho, nhập SKU từ Excel/CSV, xem trước dữ liệu đã đối chiếu rồi mới ghi nhận.">
    <main className={styles.page} data-testid="inventory-page">
      <header className={styles.header}>
        <div><p className={styles.kicker}>TỒN KHO</p><h1>Nhập tồn đầu kỳ</h1><p>Mỗi đợt nhập thuộc một kho. Nhân viên chỉ dùng SKU và mã vị trí, không cần biết ID hệ thống.</p></div>
      ...
      <section className={styles.card}>
        ...
        <p className={styles.helper}>Mẫu CSV không chứa ID kho/ID hàng. Cột bắt buộc: <strong>SKU</strong>, <strong>Số lượng</strong>. Vị trí/lô/NSX/HSD chỉ điền khi cần.</p>
      </section>
      ...
      <section className={styles.card}>
        ...
        <div className={styles.tableWrap}><table><thead><tr><th>Dòng</th><th>Kho</th><th>Vị trí</th><th>SKU</th><th>Tên hàng</th><th>Số lượng</th><th>Lô hàng</th><th>Hạn dùng</th><th>Trạng thái</th></tr></thead><tbody>
          {rows.length === 0 ? <tr><td colSpan={9} className={styles.empty}>Chọn tệp CSV để hiển thị dữ liệu.</td></tr> : effectiveRows.slice(0, 100).map((row, index) => {
            const issue = localErrors.find((item) => item.line === index + 2);
            const canonical = validation?.rows[index];
            const serverIssue = validation?.rowErrors.find((item) => item.lineNumber === index + 1);
            return <tr key={`${index}-${row.sku}-${row.locationCode}`}><td>{index + 2}</td><td>{canonical?.warehouseCode ? `${canonical.warehouseCode} — ${canonical.warehouseName ?? ''}` : selectedWarehouse ? `${selectedWarehouse.code} — ${selectedWarehouse.name}` : 'Chưa chọn'}</td><td>{canonical?.locationCode ? `${canonical.locationCode}${canonical.locationName ? ` — ${canonical.locationName}` : ''}` : row.locationCode || '—'}</td><td>{canonical?.sourceSku || row.sku || '—'}</td><td>{canonical?.productName || '—'}</td><td>{displayQuantity(canonical?.sourceQuantity || row.sourceQuantity)}</td><td>{canonical?.lotCode || row.lotCode || '—'}</td><td>{canonical?.expiryDate || row.expiryDate || '—'}</td><td>{issue ? <span className={styles.rowError}>{issue.message}</span> : serverIssue ? <span className={styles.rowError}>{serverIssue.message}</span> : validation ? <span className={styles.badgeOk}>Đã đối chiếu</span> : 'Hợp lệ sơ bộ'}</td></tr>;
          })}
        </tbody></table></div>
      ...
      <section className={styles.gridTwo}>
        <article className={styles.card}><h2>Kết quả kiểm tra</h2>{!validation ? <p className={styles.empty}>Chưa kiểm tra tệp.</p> : validation.rowErrors.length ? <ul className={styles.errorList}>{validation.rowErrors.map((item) => <li key={`${item.lineNumber}-${item.code}`}>Dòng {item.lineNumber + 1}: {item.message}</li>)}</ul> : <div className={styles.summary}><strong>{validation.totals.rowCount} dòng hợp lệ</strong><span>Tổng số lượng theo đơn vị nhập: {displayQuantity(validation.totals.sourceQuantityTotal)}</span><span>Quy đổi tồn kho: {displayQuantity(validation.totals.baseQuantityTotal)}</span><span>Kho: {selectedWarehouse ? `${selectedWarehouse.code} — ${selectedWarehouse.name}` : '—'}</span></div>}</article>
