    ...
    try {
      ...
      setMessage({
        ...
        text: result.ready
          ...
          : 'Còn dòng cần bổ sung hoặc chỉnh lại. Việc kiểm tra này chưa làm thay đổi tồn kho.',
      });
    } catch (error) {
  ...
  >
    <div className={styles.stack}>
      ...
      <section className={styles.card}>
        ...
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            <thead><tr><th>#</th><th>SKU *</th><th>Số lượng *</th><th>Giá vốn</th><th /></tr></thead>
            <tbody>{rows.map((row, index) => <tr key={index}>
              ...
              <td><button type="button" className={styles.textButton} onClick={() => { setRows((current) => current.length === 1 ? [emptyRow()] : current.filter((_, rowIndex) => rowIndex !== index)); invalidate(); }}>Xóa</button></td>
            </tr>)}</tbody>
          </table>
        </div>
      ...
      {preview ? <section className={styles.card}>
        <div className={styles.sectionHeading}>
          <div><h2>Kết quả kiểm tra</h2><p>{preview.totals.readyRowCount} dòng sẵn sàng · {preview.totals.attentionRowCount} dòng cần bổ sung · Tổng số lượng {preview.totals.sourceQuantityTotal}</p></div>
          <span className={preview.ready ? styles.readyBadge : styles.attentionBadge}>{preview.ready ? 'Sẵn sàng' : 'Cần bổ sung'}</span>
        </div>
        ...
        <div className={styles.previewList}>{preview.rows.map((row) => {
          ...
          return <article key={`${row.lineNumber}-${row.sku}`} className={row.status === 'READY' ? styles.previewReady : styles.previewAttention}>
            <div className={styles.previewTop}>
              <div><strong>{row.sku}</strong>{row.productName ? <span>{row.productName}</span> : null}<small>Số lượng {row.sourceQuantity}{row.sourceUnitCode ? ` ${row.sourceUnitCode}` : ''}{row.sourceLineNumbers.length > 1 ? ` · Gộp từ ${row.sourceLineNumbers.length} dòng` : ''}</small></div>
              <span>{row.status === 'READY' ? 'Sẵn sàng' : 'Cần bổ sung'}</span>
            </div>
            <div className={styles.facts}>
              <span>Kho: <strong>{row.warehouseCode || '—'}</strong></span>
              <span>Vị trí: <strong>{row.locationCode || (row.locationRequired ? 'Cần chọn' : 'Không bắt buộc')}</strong></span>
              <span>Lô: <strong>{row.lotTrackingMode === 'REQUIRED' ? (row.lotCode || 'Cần nhập') : 'Không quản lý'}</strong></span>
              <span>Hạn dùng: <strong>{row.expiryTrackingMode === 'REQUIRED' ? (row.expiryDate || 'Cần nhập') : row.expiryTrackingMode === 'OPTIONAL' ? (row.expiryDate || 'Tùy chọn') : 'Không quản lý'}</strong></span>
              <span>Giá vốn: <strong>{row.unitCost ? `${formatCost(row.unitCost)} đ${row.costSource === 'CURRENT' ? ' · hiện hành' : ''}` : 'Cần nhập'}</strong></span>
            </div>
