...
export default function RoleWorkspace({ initialRoles, permissions: initialPermissions, initialError = null }: Props) {
  ...
  const visibleRoles = useMemo(() => roles
    .filter((role) => {
      ...
      const rolePermissionText = role.permission_keys
        .map((key) => permissionMap.get(key)?.label ?? key)
        .join(' ');
  ...
  const shellActions = (
    <>
      ...
      <button
        ...
        onClick={openCreate}
        data-testid="roles-topbar-create-button"
      >
        Thêm vai trò
      </button>
    </>
      ...
      <section className={styles.page} data-testid="roles-page">
        ...
        <section className={styles.summaryGrid} aria-label="Số liệu vai trò">
          ...
          <article className={styles.summaryCard}>
            ...
            <small>{counts.inactive} vai trò đang ngừng hoạt động</small>
          </article>
          <article className={styles.summaryCard}>
        ...
        <section className={styles.toolbar}>
          <div className={styles.toolbarSearch}>
            ...
            <input
              ...
              placeholder="Mã, tên, mô tả, quyền…"
            />
          </div>
          <div className={styles.toolbarFilter}>
            ...
            <select
              id="roles-status"
              data-testid="roles-status-filter"
              value={statusFilter}
        ...
        <section className={styles.tableSection}>
          <div className={styles.sectionHeader}>
            <div>
              <p className={styles.panelKicker}>Danh mục vai trò</p>
              <h2>Vai trò và tập quyền</h2>
            </div>
            <span className={styles.panelChip}>{formatCompactNumber(visibleRoles.length)} vai trò</span>
          </div>
                ...
                {visibleRoles.length ? visibleRoles.map((role) => (
                  <tr key={role.id} data-testid={`role-row-${role.code}`}>
                    ...
                    <td className={styles.relationCell}>
                      <div className={styles.entityStack}>
                        <strong>{formatCompactNumber(role.permission_keys.length)} quyền</strong>
                        <span>
                          {role.permission_keys.length
                            ? role.permission_keys.map((key) => permissionMap.get(key)?.label ?? key).join(' · ')
                            : 'Chưa gán quyền'}
                        </span>
                      </div>
                    </td>
                    <td>
                      <span className={joinClasses(styles.statusPill, role.is_active ? styles.toneSuccess : styles.toneDanger)}>
                        {role.is_active ? 'Đang hoạt động' : 'Ngừng hoạt động'}
                      </span>
                    </td>
                  ...
                  <tr>
                    <td colSpan={6}>
                      <div className={styles.emptyState}>Không tìm thấy vai trò phù hợp.</div>
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className={styles.modalBackdrop} role="presentation" onClick={() => setEditor(null)}>
            <div className={joinClasses(styles.modal, matrixStyles.editorModal)} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
              <div className={styles.modalHeader}>
                <div>
                  <p className={styles.panelKicker}>{editor.mode === 'create' ? 'Vai trò mới' : 'Cập nhật vai trò'}</p>
                  <h3>{editor.mode === 'create' ? 'Thêm vai trò quản trị' : 'Chỉnh sửa vai trò quản trị'}</h3>
                </div>
              ...
              <form className={styles.form} onSubmit={(event) => void submitRole(event)}>
                <div className={matrixStyles.editorLayout}>
                  <div className={matrixStyles.editorColumn}>
                    ...
                    <label className={matrixStyles.inlineToggle}>
                      <input
                    ...
                    <div className={styles.formActions}>
                      ...
                      <button type="submit" className={styles.primaryButton} disabled={busy !== null}>
                        {busy === 'save' ? 'Đang lưu…' : editor.mode === 'create' ? 'Tạo vai trò' : 'Lưu thay đổi'}
                      </button>
                    </div>
                  </div>
                            ...
                            <div className={matrixStyles.permissionItem} key={permission.permission_key}>
                              <label>
                                <input
                                  ...
                                  data-testid={`permission-${permission.permission_key}`}
                                />
            ...
            <div className={joinClasses(styles.modal, styles.confirmModal)} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
              ...
              <p className={styles.confirmText}>
                {toggleState.nextActive
                  ? 'Vai trò sẽ trở lại trạng thái đang hoạt động.'
                  : 'Vai trò sẽ ngừng hoạt động nhưng vẫn được giữ lại để đối soát và lịch sử chứng từ.'}
              </p>
              <div className={styles.formActions}>
