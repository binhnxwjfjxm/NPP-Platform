...
function errorMessage(error?: string) {
  if (error === 'invalid_owner_code') return 'Mã xác nhận đăng nhập chưa đúng.';
...
export default function LoginPage({ searchParams }: LoginPageProps) {
  ...
  return <main className={styles.page}><section className={styles.card} aria-labelledby="npp-login-title">
    ...
    <form className={styles.form} action="/api/auth/login" method="post"><input type="hidden" name="returnTo" value={returnTo} /><label className={styles.field}><span>Tên đăng nhập hoặc email Owner</span><input name="username" autoComplete="username" required maxLength={256} autoFocus /></label><label className={styles.field}><span>Mật khẩu</span><input name="password" type="password" autoComplete="current-password" required /></label>{showOwnerCode ? <label className={styles.field}><span>Mã xác nhận đăng nhập</span><input name="ownerCode" inputMode="numeric" autoComplete="one-time-code" maxLength={6} /></label> : null}<button className={styles.submit} type="submit">Đăng nhập</button></form>
    <div className={styles.panel}><p className={styles.hint}>Bảo mật nội bộ</p><p className={styles.note}>Phiên đăng nhập gắn với đúng nhân viên, vai trò, quyền và phạm vi hiện tại trong NPP Core. Role nhạy cảm có thể yêu cầu mã xác nhận gửi tới email của chính tài khoản.</p></div><p className={styles.footer}>Không truy cập được hệ thống? Vui lòng liên hệ người quản trị tài khoản nội bộ.</p>
  </section></main>;
}
