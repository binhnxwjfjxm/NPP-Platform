...
function actionLabel(run: WarehouseLocationModeRun) {
  return run.targetMode === 'UNMANAGED' ? 'Chuyển về tồn chung' : 'Bắt đầu dùng sơ đồ kho';
}

function modeLabel(mode: WarehouseLocationModeRun['fromMode'] | WarehouseLocationModeRun['targetMode']) {
  if (mode === 'MANAGED') return 'Có sơ đồ kho';
  if (mode === 'UNMANAGED') return 'Không dùng sơ đồ';
...
function locationLabel(id: string | null, code: string | null, name: string | null) {
  ...
  return [code, name].filter(Boolean).join(' · ') || 'Khu vực đã chuyển';
}
...
export default async function WarehouseLocationModeHistoryPage({ searchParams }: Readonly<{ searchParams?: SearchParams }>) {
  ...
  try {
    ...
    error = loadError instanceof Error ? loadError.message : 'Không tải được lịch sử sơ đồ kho.';
  }
  ...
  return (
    <AppShell
      title="Kho hàng"
      subtitle="Theo dõi các lần thay đổi cách bố trí hàng hóa giữa sơ đồ kho và tồn chung."
      kicker="Danh mục quản lý"
    >
      <div className={styles.page} data-testid="warehouse-location-mode-history-page">
        <WarehouseTabs active="history" />
        ...
        <section className={styles.section}>
          <div className={styles.gridTwo}>
            <div className={styles.panel}>
              <h3 className={styles.panelTitle}>Lịch sử sơ đồ kho</h3>
              <div className={styles.tableWrap}>
                <table className={styles.table}>
                  <thead><tr><th>Thời gian</th><th>Thay đổi</th><th>Trước → Sau</th><th>SKU</th><th></th></tr></thead>
                  <tbody>
                    {runs.length === 0 ? <tr><td colSpan={5} className={styles.subtle}>Kho này chưa có lịch sử thay đổi sơ đồ.</td></tr> : runs.map((run) => (
                      <tr key={run.id}>
                        ...
                        <td><a className={styles.miniButton} href={`/organization/warehouses/location-mode-history?warehouseId=${encodeURIComponent(selectedWarehouseId)}&runId=${encodeURIComponent(run.id)}`}>Xem chi tiết</a></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <aside className={styles.panel}>
              <h3 className={styles.panelTitle}>Chi tiết lần thay đổi</h3>
              {!detail ? <p className={styles.subtle}>Chọn một lần thay đổi để xem SKU, lô, khu vực và số lượng.</p> : (
                <>
                  ...
                  <div className={styles.tableWrap}>
                    <table className={styles.table}>
                      ...
                      <tbody>
                        {(detail.lines ?? []).length === 0 ? <tr><td colSpan={5} className={styles.subtle}>Lần thay đổi này không có dòng tồn cần di chuyển.</td></tr> : detail.lines?.map((line) => (
                          <tr key={line.id}><td className={styles.mono}>{line.sku}</td><td>{line.lotCode || 'Không lô'}</td><td>{locationLabel(line.sourceLocationId, line.sourceLocationCode, line.sourceLocationName)}</td><td>{locationLabel(line.destinationLocationId, line.destinationLocationCode, line.destinationLocationName)}</td><td className={styles.mono}>{quantity(line.baseQuantity)}</td></tr>
                        ))}
                      </tbody>
                    </table>
                  </div>
