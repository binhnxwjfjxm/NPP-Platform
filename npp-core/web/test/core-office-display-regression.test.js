...
test('Công Ty screen typography is reduced to 80 percent while print and main control hit areas stay stable', () => {
  assert.match(layoutSource, /import '\.\/core-office-density\.css';/);
  assert.match(densityCss, /html\s*\{\s*font-size:\s*80%;/);
  assert.match(densityCss, /body\s*\{\s*font-size:\s*11\.2px;/);
