...
test('Issue #1110 Lô 8 monthly matrix separates Nghỉ, Phép, Vắng, Thiếu and pending work', async () => {
  ...
  assert.match(workspace, />Vắng<\/strong> Có lịch làm nhưng không có chấm công hoặc đơn nghỉ được duyệt/);
  assert.match(workspace, />Chưa đủ<\/strong> Có chấm công nhưng chưa đầy đủ/);
  assert.match(workspace, />Chờ duyệt nghỉ<\/strong> Đơn nghỉ đang chờ duyệt/);
...
test('Issue #1110 Lô 8 day detail shows shift segment, leave, counted leave and control facts', async () => {
  ...
  assert.match(workspace, /Thời gian phải làm/);
  assert.match(workspace, /Phần được nghỉ/);
  assert.match(workspace, /Thời gian nghỉ được tính công/);
