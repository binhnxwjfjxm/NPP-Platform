...
const coreSafePattern = /^[A-Za-z0-9._-]{1,128}$/;
const masterCreateCoreKey = (value) => coreSafePattern.test(value) ? value : createHash('sha256').update(value, 'utf8').digest('hex');

test('A1-A3 logistics master create translates only Core-invalid UI idempotency keys and preserves safe retry keys', () => {
  ...
  assert.ok(gatewaySource.includes("const CORE_SAFE_IDEMPOTENCY_KEY_PATTERN=/^[A-Za-z0-9._-]{1,128}$/;"));
  assert.ok(gatewaySource.includes("const MASTER_CREATE_RESOURCES=new Set(['routes','vehicles','drivers']);"));
  assert.ok(gatewaySource.includes("function masterCreateCoreKey(v:string|null|undefined){const candidate=key(v);return CORE_SAFE_IDEMPOTENCY_KEY_PATTERN.test(candidate)?candidate:createHash('sha256').update(candidate,'utf8').digest('hex');}"));
  ...
  for (const sourceKey of ['vehicle:create', 'driver:create', 'route:create']) {
    ...
    assert.match(coreKey, coreSafePattern);
    assert.equal(coreKey.length, 64);
    assert.equal(masterCreateCoreKey(sourceKey), coreKey);
  }

  for (const safeKey of ['vehicle_create', 'driver.create', 'route-create']) {
    assert.equal(masterCreateCoreKey(safeKey), safeKey);
  }
