...
test('return credit page explains received-return settlement without changing warehouse receipt', () => { assert.match(page, /title="Điều chỉnh công nợ hàng trả"/); assert.match(page, /Khoản giảm công nợ chỉ phát sinh khi kho đã nhận phiếu hàng khách trả/); assert.match(page, /CustomerReturnCreditWorkspace/); assert.doesNotMatch(page, /Credit chỉ phát sinh|Customer Return|chi tiết credit|\bCOD\b|MCP|write[-_ ]?off/i); });
test('workspace supports allocation explicit refund and compensating reversal', () => { assert.match(workspace, /data-testid="customer-return-credit-workspace"/); assert.match(workspace, /data-testid="customer-return-credit-allocation-form"/); assert.match(workspace, /data-testid="customer-refund-form"/); assert.match(workspace, /Phân bổ công nợ/); assert.match(workspace, /Ghi nhận hoàn tiền/); assert.match(workspace, /Đảo hoàn tiền/); assert.match(workspace, /Đảo khoản giảm công nợ hàng trả/); assert.match(workspace, /destinationReference/); assert.match(workspace, /reversalReason/); assert.doesNotMatch(workspace, /Phân bổ credit|Đảo credit|Credit từ hàng khách trả|Customer Return|auto.*refund|\bCOD\b|MCP|write[-_ ]?off/i); });
