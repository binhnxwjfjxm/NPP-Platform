...
test('shared print foundation isolates only the selected A4/A5 document from application layout', () => {
  ...
  assert.match(printSource, /data-print-root/);
  ...
  assert.match(globalCss, /body\[data-printing='true'\] > \*:not\(\[data-print-root='true'\]\)/);
  assert.match(globalCss, /display:\s*none !important/);
  assert.doesNotMatch(globalCss, /body\[data-printing='true'\] \*\s*\{\s*visibility:\s*hidden/);
  assert.doesNotMatch(globalCss, /body:has\(\[data-print-surface\]\)/);
});

test('shared print action uses the light bronze-gold operation tone', () => {
  assert.match(printCss, /#f3e6cf/);
  assert.match(printCss, /#754706/);
  assert.match(printCss, /#98600f/);
});
...
test('purchase and sales documents omit zero discount and tax sections', () => {
  for (const source of [poPrint, salesPrint]) {
    assert.match(source, /isZeroAmount/);
    assert.match(source, /showDiscount/);
...
test('canonical print surfaces remain available', () => {
  ...
  assert.match(deliveryPrint, /PHIẾU ĐÓNG GÓI/);
...
test('detail-based print controls do not maintain a second selector/fetch state', () => {
  for (const source of contextualModules) {
    assert.doesNotMatch(source, /DocumentPrintDock/);
    assert.doesNotMatch(source, /fetch\(/);
    assert.doesNotMatch(source, /selectedId/);
  }
});
