...
async function mockReconciliationApis(page: Page) {
  let returned = false;
  let closed = false;
  const tripList = () => [{
    id: tripId,
  ...
  const detail = () => ({
    ...
    lines: [{
      ...
      returnedBaseQuantity: returned ? '2.000000000000' : '0.000000000000',
      outstandingBaseQuantity: returned ? '0.000000000000' : '2.000000000000',
    }],
    receipts: returned ? [{
  ...
  await page.route(`**/api/logistics/trips/${tripId}/return-receipts`, async (route) => {
    ...
    expect(body.lines).toEqual([{ inventoryIssueLineId: issueLineId, returnedBaseQuantity: '2.000000000000' }]);
    returned = true;
