...
async function fulfill(route: Route, data: unknown, status = 200) {
  await route.fulfill({ status, json: { data, requestId: 'e2e-phase-10-lane-a' } });
}

async function fulfillError(route: Route, code: string, message: string, status = 400) {
  await route.fulfill({ status, json: { error: { code, message, retryable: false, details: {} }, requestId: 'e2e-phase-10-lane-a' } });
}

type State = { created: boolean; assigned: boolean; status: 'draft' | 'planned' | 'locked'; revision: number; keys: string[] };

function eligibleOrder() {
  return {
    id: deliveryOrderId, number: 'DO-202608-000010', salesOrderId: '99999999-9999-4999-8999-999999999999', warehouseId,
...
function trip(state: State) {
  return {
    ...
    status: state.status, note: 'Chuyến sáng', revision: String(state.revision), stopCount: state.assigned ? 1 : 0, assignmentCount: state.assigned ? 1 : 0,
    stops: state.assigned ? [{
      ...
      assignments: [{ assignmentId, deliveryOrderId, deliveryOrderNumber: 'DO-202608-000010', customerCode: 'KH-001', customerName: 'Cửa hàng Minh Anh', requestedDeliveryDate: '2026-08-05', collectionPolicy: 'PREPAID' }],
    }] : [],
  };
}
...
async function mockLogisticsApis(page: Page, state: State) {
  ...
  await page.route('**/api/logistics/eligible-delivery-orders**', (route) => fulfill(route, state.assigned ? [] : [eligibleOrder()]));
  await page.route('**/api/logistics/trips', async (route) => {
    if (route.request().method() === 'POST') {
      state.created = true; state.keys.push(route.request().headers()['idempotency-key'] || '');
      await fulfill(route, { ok: true, replayed: false, trip: trip(state) }, 201); return;
    }
    await fulfill(route, state.created ? [trip(state)] : []);
  });
  await page.route(`**/api/logistics/trips/${tripId}`, async (route) => {
    if (route.request().method() === 'PUT') {
      state.revision += 1; state.keys.push(route.request().headers()['idempotency-key'] || '');
      await fulfill(route, { ok: true, replayed: false, trip: trip(state) }); return;
    }
    await fulfill(route, trip(state));
  });
  await page.route(`**/api/logistics/trips/${tripId}/*`, async (route) => {
    const action = route.request().url().split('/').at(-1); state.keys.push(route.request().headers()['idempotency-key'] || '');
    ...
    state.revision += 1; await fulfill(route, { ok: true, replayed: false, trip: trip(state) });
  });
}
