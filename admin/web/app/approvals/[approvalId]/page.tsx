...
export default function ApprovalDetailPage({ params }: { params: { approvalId: string } }) {
  ...
  return (
    <AdminShell activeSection="approvals" title="Chi tiết đề xuất" subtitle="Xem đầy đủ tác động, quy tắc và bằng chứng trước khi ra quyết định.">
      <Link className="approvalBackLink" href="/approvals">← Quay lại danh sách</Link>
      <p className="adminPreviewNotice">Màn này đang dùng dữ liệu mẫu frontend. Các nút quyết định chưa gửi yêu cầu tới backend.</p>
      ...
      <section className="approvalDetailSection card"><h3>Quy tắc liên quan</h3><p>{item.rule}</p></section>
      ...
      {item.state === 'pending' ? <section className="approvalDecisionBar" aria-label="Hành động quyết định"><button type="button" disabled>Phê duyệt</button><button type="button" disabled>Yêu cầu bổ sung</button><button type="button" disabled>Từ chối</button><small>Chức năng quyết định sẽ được kích hoạt khi backend contract được nối ở phase tích hợp.</small></section> : null}
    </AdminShell>
  );
}
