  ...
  return (
    <div className="shell adminAppShell" data-admin-app-shell data-active-section={activeSection ?? 'secondary'}>
      <header className="topbar adminAppTopbar"><div className="topbarInner">
        <Link className="brand" href="/" aria-label="Admin MCP/NPP - Tổng quan"><span className="brandLogoFrame"><img className="brandLogo" src={appLogoUrl} alt="Logo Hưng Phát Company" /></span><span className="brandCopy"><strong className="brandProductName">Admin MCP/NPP</strong><span className="brandDescriptor">Theo dõi và quyết định quản trị</span><small className="brandMobileEyebrow">Admin Hưng Phát</small><strong className="brandScreenName">{title}</strong></span></Link>
        ...
        <details className="appMenu adminAccountMenu"><summary className="menuTrigger" aria-label="Mở menu tài khoản"><AdminIcon name="user" size={22} /></summary><div className="menuPanel"><p className="menuEyebrow">Tài khoản</p><Link className="menuItem" href="/menu"><span className="menuIcon"><AdminIcon name="info" size={20} /></span><span><strong>Thông tin ứng dụng</strong><small>Cài đặt ứng dụng và phạm vi sử dụng</small></span></Link><form action="/api/auth/logout" method="post"><button className="menuItem adminLogoutItem" type="submit"><span className="menuIcon"><AdminIcon name="lock" size={20} /></span><span><strong>Đăng xuất</strong><small>Kết thúc phiên quản trị hiện tại</small></span></button></form></div></details>
      </div></header>
