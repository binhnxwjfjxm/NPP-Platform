...
import {
  AdminFilterChip, AdminKpiCard, AdminKpiGrid, AdminStatePanel, AdminStatusBadge, AdminToolbar,
  type AdminStateTone, type AdminStatusTone,
} from "../admin-ui-primitives";
import { useAdminLocalRead } from "../local-read/use-admin-local-read";
import type { LotCPresentation } from "./report-lot-c-data";
import styles from "./report-center.module.css";

type ReportDomain = "executive" | "debt" | "inventory" | "delivery-cod" | "mcp" | "people" | "decisions";
type ReportPeriod = "Hôm nay" | "7 ngày" | "Tháng này" | "Quý này";
type ReportState = "ready" | "partial" | "empty" | "forbidden" | "error" | "unavailable";
...
function reportHref(tab: ReportDomain, period: ReportPeriod, warehouseId?: string | null): string {
...
export function ReportsLocal({ selected, period, warehouseId, from, to }: { selected: ReportDomain; period: ReportPeriod; warehouseId: string | null; from: string; to: string }) {
  const read = useAdminLocalRead<LotCPresentation>("reports", period, { tab: selected, warehouseId });
  ...
  if (selected !== "debt") { exportParams.set("from", from); exportParams.set("to", to); }
  ...
  return (
    <AdminShell activeSection="reports" title="Báo cáo quản trị" subtitle="Theo dõi số liệu quản trị từ Công Ty và MCP trong phạm vi quyền hiện tại.">
      ...
      {!item ? <AdminStatePanel className={styles.reportState} title="Đang mở báo cáo" message="Dữ liệu đã lưu sẽ hiện ngay khi có trên thiết bị; bản mới đang được cập nhật nền." tone="partial" /> : <ReportBody item={item} selected={selected} period={period} warehouseId={warehouseId} />}
    </AdminShell>
  );
}

function ReportBody({ item, selected, period, warehouseId }: { item: LotCPresentation; selected: ReportDomain; period: ReportPeriod; warehouseId: string | null }) {
  ...
  return <>
    ...
    <section className={`card ${styles.hero}`}><div className={styles.heroCopy}><span className={styles.eyebrow}>{item.domainLabel} · {item.periodLabel}</span><h2>{item.title}</h2><p>{item.summary}</p></div><div className={styles.comparison}><small>{item.primary.label}</small><strong>{item.primary.value}</strong><AdminStatusBadge tone={reportStatusTone(item.state)} className={styles.reportStatusBadge}>{item.stateLabel}</AdminStatusBadge><small>Phạm vi thời gian</small><b>{item.periodLabel}</b></div></section>
    ...
    <section className={`card ${styles.trend}`}><div className={styles.sectionHeading}><div><span>Xu hướng kỳ</span><h3>Diễn biến từ số liệu thật</h3></div>{item.trendLabel ? <strong>{item.trendLabel}</strong> : null}</div>{item.trend.length > 0 ? <><div className={styles.sparkBars} aria-label={item.trendLabel ?? "Diễn biến kỳ"}>{item.trend.map((point) => { const height = trendMax > 0 ? Math.max(8, Math.round((point.value / trendMax) * 100)) : 8; return <span key={`${point.label}-${point.display}`} style={{ height: `${height}%` }} title={`${point.label}: ${point.display}`} />; })}</div><div className={styles.detailRows} aria-label="Giá trị xu hướng theo ngày">{item.trend.map((point) => <div key={`visible-${point.label}-${point.display}`}><span>{point.label}</span><strong>{point.display}</strong></div>)}</div><p className={styles.detailNote}>{item.trendNote}</p></> : <p className={styles.detailNote}>{item.trendNote}</p>}</section>
