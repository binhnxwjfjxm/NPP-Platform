...
import type { BusinessBreakdownKey, BusinessReport, BusinessRow } from '../business-report-data';
...
function replaceWorkspaceUrl(dimension: BusinessBreakdownKey, item?: string | null) {
...
export function BusinessReportWorkspace({
  report,
  periods,
  initialDimension,
  initialItem,
  ...
  initialDimension: BusinessBreakdownKey;
  initialItem?: string;
}) {
  const [selectedDimension, setSelectedDimension] = useState<BusinessBreakdownKey>(initialDimension);
  ...
  const selectedRowIndex = rows.findIndex((row, index) => rowToken(row, index) === selectedItem);
  const selectedRow = selectedRowIndex >= 0 ? rows[selectedRowIndex] : null;

  function selectDimension(value: BusinessBreakdownKey) {
    if (value === selectedDimension) return;
    setSelectedDimension(value);
    setSelectedItem('');
    replaceWorkspaceUrl(value);
  }

  function selectRow(row: BusinessRow, index: number) {
    const token = rowToken(row, index);
    setSelectedItem(token);
    replaceWorkspaceUrl(selectedDimension, token);
  }

  function closeDetail() {
    setSelectedItem('');
    replaceWorkspaceUrl(selectedDimension);
  }
  ...
  if (selectedDimension === 'customers' && report.filters.customerGroupId) exportQuery.set('customerGroupId', report.filters.customerGroupId);
  ...
  const dimensionFilter = (placement: 'desktop' | 'mobile') => {
    if (selectedDimension === 'customers') {
      return (
        <form className={`${styles.dimensionFilters} ${placement === 'desktop' ? styles.desktopDimensionFilters : styles.mobileDimensionFilters}`} method="get" action="/reports/business" aria-label="Lọc khách hàng theo nhóm">
    ...
    if (selectedDimension === 'products') {
      return (
        <form className={`${styles.dimensionFilters} ${placement === 'desktop' ? styles.desktopDimensionFilters : styles.mobileDimensionFilters}`} method="get" action="/reports/business" aria-label="Lọc sản phẩm theo nhóm">
          <input type="hidden" name="period" value={report.period} />
  ...
  return (
    <div className={styles.workspace} data-testid="admin-business-report-workspace">
      <AdminToolbar
        label="Kỳ báo cáo"
        actions={
      ...
      <section className={`card ${styles.analysisPanel}`} aria-labelledby="business-analysis-title">
        <div className={styles.analysisHeader}>
          <div>
            <h2 id="business-analysis-title">Chi tiết kinh doanh</h2>
            <small>{selectedDimensionLabel}</small>
          </div>
          <Link className={styles.profitLink} href="/reports/profit">Xem Lợi nhuận</Link>
        </div>

        <div className={styles.desktopAnalysisToolbar}>
          <label className={styles.desktopDimensionControl}>
            <span>Chiều phân tích</span>
            <select value={selectedDimension} onChange={(event) => selectDimension(event.target.value as BusinessBreakdownKey)}>
              {dimensions.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
            </select>
          </label>
          <div className={styles.desktopFilterSlot}>{dimensionFilter('desktop')}</div>
        </div>
        ...
        <div className={styles.mobileFilterSlot}>{dimensionFilter('mobile')}</div>
                    ...
                    {rows.map((row, index) => {
                      ...
                      return (
                        <tr key={rowToken(row, index)} className={selected ? styles.selectedRow : undefined}>
                          <td>
                            <button type="button" className={styles.rowButton} onClick={() => selectRow(row, index)}>
          ...
          {selectedRow ? (
            <aside className={styles.detailPanel} aria-label={`Chi tiết ${rowName(selectedRow, selectedDimension)}`}>
              <div className={styles.detailHeader}>
                ...
                <button type="button" onClick={closeDetail} className={styles.closeDetail} aria-label="Đóng chi tiết">×</button>
              </div>
