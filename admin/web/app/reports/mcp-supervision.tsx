...
export async function McpSupervision({ period, viewInput }: { period: ReportPeriod; viewInput?: string }) {
  ...
  return <section className={`card ${styles.detailSection}`}>
    ...
    {result.data && view === 'anomalies' ? <div className={styles.detailHighlights}>{anomalies.length ? anomalies.map((row,index)=>{
      const alertId = text(row,'id','');
      return <div key={`${alertId || index}-${index}`}><strong>{text(row,'title','Cần kiểm tra')}</strong><br/>{text(row,'entity','Điểm bán')} · {text(row,'actual','')}{alertId ? <><br/><Link href={`/alerts/${encodeURIComponent(alertId)}?period=${encodeURIComponent(period)}`}>Mở cảnh báo</Link></> : null}</div>;
    }) : <div>Không phát hiện bất thường MCP trong kỳ đang xem.</div>}</div> : null}
  </section>;
}
