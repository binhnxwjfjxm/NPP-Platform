...
export default async function AlertDetailPage({ params }: { params: Promise<{ alertId: string }> }) {
  ...
  if (!alert) notFound();
  return <AdminShell activeSection="alerts" title="Chi tiết cảnh báo" subtitle="Thông tin quy tắc, ngưỡng và tín hiệu thực tế.">
    <Link className="approvalBackLink" href="/alerts">← Trung tâm cảnh báo</Link>
    <p className="adminPreviewNotice">Dữ liệu minh họa. Chưa có thao tác ghi nhận hoặc thay đổi quy tắc.</p>
    ...
    <section className="card alertDetailSection"><h3>Tín hiệu cảnh báo</h3><div className="alertComparison"><div><small>Ngưỡng quy tắc</small><strong>{alert.threshold}</strong></div><span>→</span><div><small>Giá trị thực tế</small><strong>{alert.actual}</strong></div></div></section>
    <section className="card alertDetailSection"><h3>Quy tắc liên quan</h3><dl className="approvalDefinitionList"><div><dt>Mã quy tắc</dt><dd>{alert.ruleCode}</dd></div><div><dt>Tên quy tắc</dt><dd>{alert.ruleName}</dd></div><div><dt>Phát hiện lúc</dt><dd>{alert.detectedAt}</dd></div><div><dt>Nguồn dữ liệu</dt><dd>{alert.source}</dd></div></dl></section>
