...
export default async function AlertsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  return <AdminShell activeSection="alerts" title="Trung tâm cảnh báo" subtitle="Theo dõi tín hiệu bất thường theo quy tắc quản trị.">
    ...
    <div className="alertSummaryStrip"><div><span>Đang hoạt động</span><strong>{activeAlerts.length}</strong></div><div><span>Nghiêm trọng / Cao</span><strong>{activeAlerts.filter(a=>a.severity==='critical'||a.severity==='high').length}</strong></div><div><span>Rule đang hiển thị</span><strong>{alertRulesPreview.length}</strong></div></div>
    <p className="adminPreviewNotice">Dữ liệu dưới đây là dữ liệu mẫu frontend để khóa trải nghiệm quản trị. Chưa phải dữ liệu production và chưa có cấu hình rule được ghi vào backend.</p>
    {activeTab === 'rules' ? <section className="alertRuleList" aria-label="Quy tắc cảnh báo mẫu">{alertRulesPreview.map(rule=><article className="card alertRuleCard" key={rule.code}><div><span className="alertRuleCode">{rule.code}</span><span className="alertSeverity is-attention">{rule.severity}</span></div><h2>{rule.name}</h2><p>{rule.domain} · {rule.condition}</p><small>Chỉ xem trước — chưa thể chỉnh sửa rule.</small></article>)}</section> : activeTab === 'history' ? <section className="card alertEmpty"><strong>Lịch sử cảnh báo</strong><span>Lịch sử thật sẽ hiển thị khi backend cung cấp trạng thái đã ghi nhận/đã xử lý. Stage này không tạo lịch sử giả.</span></section> : <section className="alertList" aria-label="Danh sách cảnh báo">{filtered.map(alert=><Link className="card alertListItem" href={`/alerts/${alert.id}`} key={alert.id}><div className="alertListTopline"><span className={`alertSeverity is-${alert.severity}`}>{alertSeverityLabels[alert.severity]}</span><span className="alertStatus">Đang hoạt động</span></div><h2>{alert.title}</h2><p className="alertEntity">{alert.entity}</p><div className="alertMetricGrid"><span><small>Rule</small><strong>{alert.ruleCode}</strong></span><span><small>Ngưỡng</small><strong>{alert.threshold}</strong></span><span><small>Thực tế</small><strong>{alert.actual}</strong></span><span><small>Nguồn</small><strong>{alert.source}</strong></span></div><div className="alertListFooter"><span>{alert.detectedAt}</span><strong>Xem chi tiết →</strong></div></Link>)}</section>}
  </AdminShell>;
}
