...
test('alert center uses live MCP data, canonical lifecycle and office language', async () => {
  ...
  for (const label of ['Nghiêm trọng','Cao','Cần chú ý','Quy tắc','Ngưỡng','Thực tế','Nguồn']) assert.match(page, new RegExp(label));
  for (const label of ['Mới','Đã xem','Đang xử lý','Đã giải quyết']) assert.match(`${page}\n${detail}`, new RegExp(label));
  assert.match(page,/alertRuleList/); assert.match(detail,/Tín hiệu cảnh báo/); assert.match(detail,/Bằng chứng hiện có/); assert.match(detail,/Hướng rà soát/); assert.match(detail,/Lịch sử xử lý/); assert.match(data,/\/api\/reporting\/admin-alerts/); assert.match(data,/server-only/); assert.match(action,/idempotencyKey/); assert.match(action,/method: 'POST'/); assert.match(css,/\.alertComparison/); assert.match(css,/\.alertSeverity\.is-critical/); assert.doesNotMatch(`${page}\n${detail}`, /alert-preview-data|Dữ liệu minh họa|frontend|backend|production|Stage|Mã rule|Tên rule|Rule đang/i);
});
