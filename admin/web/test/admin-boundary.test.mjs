...
test('admin keeps standalone frontend with shared Công Ty backend and auth', async () => {
...
test('admin shell exposes exactly four management destinations with proposal wording', async () => {
  ...
  assert.doesNotMatch(shell, /NPP_OPERATIONS_URL|NPP Operations/);
  assert.doesNotMatch(shell, /label="Menu"|section: 'menu'/);
  assert.match(tabs, /adminIconTabs/);
  assert.match(tabs, /adminIconTabBadge/);
  ...
  assert.match(overview, /href="\/approvals"/);
  assert.match(overview, /href="\/alerts"/);
  assert.match(overview, /href="\/reports"/);
  assert.doesNotMatch(overview, /NPP_OPERATIONS_URL|npp-platform\.vercel\.app/);
});

test('admin main read surfaces keep management taxonomy without browser-side business mutations', async () => {
  const [proposalWrapper, proposals, alertWrapper, alerts, reports] = await Promise.all([
  ...
  assert.doesNotMatch(`${proposalWrapper}\n${proposals}\n${alertWrapper}\n${alerts}\n${reports}`, /requestCore|method:\s*["'](?:POST|PUT|PATCH|DELETE)["']|Idempotency-Key/);
});
...
test('proposal center keeps reading separate from the decision dialog', async () => {
  ...
  assert.match(detail,/Lý do \/ bối cảnh/);
  assert.match(detail,/Điều kiện cần lưu ý/);
  assert.match(detail,/Dữ liệu và bằng chứng/);
  assert.match(detail,/item\.reason \?/);
  assert.match(detail,/item\.rule \?/);
  assert.match(detail,/item\.evidence\.length \?/);
  assert.match(detail,/hasRelatedEntity \?/);
  ...
  assert.match(dialog,/required=\{noteRequired\}/);
  assert.match(dialog,/disabled=\{!decision\}/);
  assert.match(dialog,/Xác nhận đồng ý/);
  assert.match(dialog,/Gửi yêu cầu bổ sung/);
  assert.match(dialog,/Xác nhận từ chối/);
  assert.match(dialogCss,/\.dialog::backdrop/);
  assert.match(dialogCss,/\.decisionGrid/);
  assert.match(dialogCss,/@media \(max-width: 640px\)/);
  assert.match(fixtures,/source: 'Công Ty'/);
  assert.match(fixtures,/source: 'MCP'/);
  assert.doesNotMatch(fixtures,/domain: 'inventory'|domain: 'delivery-cod'|source: 'Core'/);
  assert.doesNotMatch(`${page}\n${detail}\n${dialog}\n${fixtures}`, /requestCore|method=['"]?(POST|PUT|PATCH|DELETE)|Idempotency-Key/);
});

test('alert center uses live multi-domain data, canonical lifecycle and office language', async () => {
  const [page, detail, data, action, css] = await Promise.all([
    read('app/alerts/alerts-local.tsx'),
    read('app/alerts/[alertId]/page.tsx'),
    read('app/alerts/alert-data.ts'),
  ...
  assert.match(detail,/Tín hiệu cảnh báo/);
  assert.match(detail,/Bằng chứng hiện có/);
  assert.match(detail,/Hướng rà soát/);
  assert.match(detail,/Lịch sử xử lý/);
  assert.match(detail,/AdminActionBar/);
  assert.match(detail,/AdminStatusBadge/);
  assert.doesNotMatch(detail,/approvalDecisionBar/);
  assert.match(data,/\/api\/reporting\/admin-alerts/);
  ...
  assert.match(css,/\.alertComparison/);
...
test('management reports use office language and server-side Công Ty/MCP reporting sources', async () => {
  ...
  assert.doesNotMatch(page,/Dữ liệu minh họa|report-preview-data/);
  assert.match(detail,/Phạm vi số liệu/);
  assert.match(detail,/Chỉ số quản trị/);
  assert.match(detail,/Nguồn số liệu/);
  assert.match(data,/Công Ty/);
  assert.match(data,/MCP/);
  assert.match(data,/requestCore/);
  assert.match(data,/server-only/);
  assert.doesNotMatch(`${page}\n${detail}`, /frontend|backend|production|contract|phase/i);
  assert.match(css,/\.sparkBars/);
  assert.match(css,/grid-template-columns:repeat\(2,minmax\(0,1fr\)\)/);
  assert.doesNotMatch(`${page}\n${detail}\n${data}`, /method=['"]?(POST|PUT|PATCH|DELETE)|Idempotency-Key/);
});

test('overview uses local-first snapshots backed by live proposal alert and reporting sources', async () => {
  const [wrapper, overview, route, hook, controlTower, managementCss, overviewCss, mobileApp, foundation] = await Promise.all([
    read('app/page.tsx'),
    read('app/admin-overview-local.tsx'),
  ...
  assert.match(wrapper, /AdminOverviewLocal/);
  assert.match(overview, /useAdminLocalRead<AdminControlTowerData>\("control-tower", period\)/);
  assert.match(overview, /useAdminLocalRead<ProposalItem\[]>\("proposals", period\)/);
  assert.match(overview, /useAdminLocalRead<AlertCenterData>\("alerts", period\)/);
  ...
  assert.match(overview, /reportPeriods\.map/);
  assert.match(overview, /executiveReportState/);
  ...
  assert.match(controlTower, /URLSearchParams/);
  ...
  assert.match(overview, /Một số số liệu chưa đầy đủ/);
  assert.match(overview, /Không có việc ưu tiên đang mở/);
  assert.doesNotMatch(overview,/Phê duyệt|dữ liệu mẫu frontend|backend|production|contract|phase/i);
  assert.doesNotMatch(overview,/overviewDecisionStrip|styles\.periodTabs|styles\.metricLink/);
  assert.match(overview,/overviewFocusList/);
  assert.doesNotMatch(overviewCss,/\.periodTabs|\.metricLink|\.periodMeta/);
  assert.match(overviewCss,/\.sourceState/);
  assert.match(overviewCss,/\.focusState/);
  assert.match(mobileApp,/grid-template-rows:\s*auto minmax\(0,\s*1fr\) auto/);
  assert.doesNotMatch(mobileApp,/grid-template-rows:\s*auto minmax\(0,\s*1fr\) calc\(64px \+ env\(safe-area-inset-bottom\)\)/);
  assert.match(mobileApp,/padding:\s*5px 8px max\(5px,\s*env\(safe-area-inset-bottom\)\)/);
  assert.match(foundation,/\.adminPageHeader h1 \{ font-size: 1\.42rem; \}/);
