...
const reportsLocalPath = new URL('../app/reports/ReportsLocal.tsx', import.meta.url);
...
test('Admin Lô C opens existing reporting detail for inventory, gross margin, COD and decisions', async () => {
  ...
  for (const endpoint of ['/api/reporting/inventory', '/api/reporting/gross-margin', '/api/reporting/cod', '/api/management-proposals', '/api/reporting/admin-alerts']) assert.match(drilldown, new RegExp(endpoint.replaceAll('/', '\\/')));
...
test('Admin Lô C warehouse filter uses canonical warehouseId only where supported', async () => {
  const [data, page, local, detail] = await Promise.all([readFile(dataPath, 'utf8'), readFile(reportsPagePath, 'utf8'), readFile(reportsLocalPath, 'utf8'), readFile(detailPath, 'utf8')]);
  ...
  assert.match(local, /Tất cả kho/);
  assert.match(detail, /warehouseId/);
  assert.doesNotMatch(`${page}\n${local}`, /branchId|employeeId|customerId|channelId/);
});

test('Admin Lô C debt is visibly current balance and no longer offers a historical period selector', async () => {
  const [data, local] = await Promise.all([readFile(dataPath, 'utf8'), readFile(reportsLocalPath, 'utf8')]);
  assert.match(data, /Phạm vi thời gian', value: 'Số dư hiện tại'/);
  assert.match(local, /selected === "debt"/);
  assert.match(local, />Số dư hiện tại</);
});

test('Admin Lô C alert history shows resolved actor and report deep-links preserve return flow', async () => {
  const [alertData, alertDetail, proposalDetail] = await Promise.all([readFile(alertDataPath, 'utf8'), readFile(alertDetailPath, 'utf8'), readFile(proposalDetailPath, 'utf8')]);
