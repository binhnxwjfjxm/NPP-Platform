...
const overviewLocalPath = new URL('../app/admin-overview-local.tsx', import.meta.url);
...
test('Admin reports use live Company reporting sources instead of preview fixtures', async () => {
  const [overviewPage, overviewLocal, page, detail, data] = await Promise.all([
    readFile(overviewPagePath, 'utf8'),
    readFile(overviewLocalPath, 'utf8'),
  ...
  assert.match(overviewLocal, /Số liệu Công Ty đã sẵn sàng/);
