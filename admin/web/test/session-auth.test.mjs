...
const logoutForm = read('app/admin-logout-form.tsx');
...
test('Admin login and logout proxy the canonical Core internal-auth lifecycle without exposing the token to browser JavaScript', () => {
  ...
  assert.match(logoutForm, /action="\/api\/auth\/logout"/);
  assert.match(logoutForm, /clearAdminLocalReadForCurrentUser\(\)/);
  assert.match(logoutForm, /Đăng xuất/);
  assert.doesNotMatch(loginRoute + logoutRoute + logoutForm, /CORE_API_SERVER_TOKEN|CORE_WEB_ADMIN_USERNAME|CORE_WEB_ADMIN_PASSWORD/);
});
