...
test('Admin displays the Hưng Phát logo with an environment override and local fallback', () => {
  assert.match(shellSource, /NEXT_PUBLIC_APP_LOGO_URL/);
  assert.match(shellSource, /'\/icons\/admin-192\.png'/);
  assert.doesNotMatch(shellSource, /office\.nguyenlieuhungphat\.com\/logo-transparent\.png/);
